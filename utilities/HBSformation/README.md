...
| `phasefield_prototype_1d.py` | 1-D phase-field prototype of the same functional, with a gradient term; exploratory, not the model SCIANTIX runs |
| `phasefield_tandogan_1d.py` | 1-D HMP orientation phase field of Tandogan et al. (JMPS 2026, Sec. 2), driven by `rho_free`, `Theta` and `G` from the Landau model; checked against the paper's Cu bicrystal; `--irradiation` replaces the plastic dislocation source with an irradiation one |
...
# the exploratory 1-D phase-field prototype:
python3 phasefield_prototype_1d.py --selftest --plots

# the Tandogan-type prototype (about 3 min for the whole set):
python3 phasefield_tandogan_1d.py --selftest --cu --case 90 --scan
python3 phasefield_tandogan_1d.py --irradiation --bu-rate 1e-3   # ~1 min
```
...
`rho_ord > rho_tot`: the walls are asked to hold more dislocations than exist. The state
returned by `hbs_state()` carries `balance_limited`, which is `True` exactly where the bound
binds rather than the stationary point — over roughly 65.6–91.3 GWd/tU at `T = 600 K,
P = 0.05`, and at 11 of the 42 EBSD points:
...
| `beta` | 26.681642770151456 | - | `calibrate.py`, joint fit (data set C, w_r = 0.2, w_X = 1) |
| `k` | 0.5102476353172847 | - | idem |
| `rho_c` | 5741121567363591.0 | m⁻² | idem; `rho_c^(-1/2)` = 0.013 µm |
...
`calibrate.py --weight W` reproduces any row of that scan.
...
calibrated on the JSON datasets (data set C, w_r = 0.2, w_X = 1) the dislocation balance of
Eq. (7b) **never binds**.
...
| X(60 GWd/tU) | 0.29452 | 0.29452 | 0.29452 | 0.29452 | 0.29452 |
...
model — the useful cross-check: they agree to 0.1 MWd/kgU at the 1 % crossing. The two
...
|---|---|---|---|---|
| overshoot | 0.151897 at 106.8 | 0.160076 at 117.0 | none | **0.161225 at 91.4** |
| dip after it | −0.6 %, to 0.151002 at 122.5 | −3.0 %, to 0.155196 at 144.1 | +0.0 % | **−8.4 %**, to 0.147720 at 114.1 |
| final | 0.165361 | 0.165306 | 0.165592 | 0.165388 |
...
| 4 Landau functional | **0.1942** | **+0.723** | **0.1842** | **+0.744** | calibrated on all of them |
...
|---|---|---|---|---|
| Zacharie (2022), std UO₂ | 27 | 69.4 % | 0.1976 | 0.1948 |
| Zacharie (2022), Cr-doped | 11 | 19.4 % | 0.2048 | 0.2048 |
| Gerczak (2018) | 5 | 3.3 % | 0.1033 | 0.1033 |
| Noirot (2015) | 6 | 7.9 % | 0.1640 | 0.1434 |
...
it scores 0.120 / +0.76.

| paper | bu [MWd/kgU] | T [K] | rank | w | measured | opt 4 | opt 4 − measured |
|---|---|---|---|---|---|---|---|
| GER2018 | 64.7 | 978 | U | 0.25 | 0.150 | 0.154 | +0.004 |
| GER2018 | 71.2 | 845 | U | 0.25 | 0.410 | 0.223 | **−0.187** |
| NOI2015 | 72.0 | 1023 | B | 0.33 | 0.167 | 0.233 | +0.065 |
| NOI2015 | 72.0 | 1023 | B | 0.33 | 0.212 | 0.233 | +0.021 |
| NOI2015 | 72.0 | 1023 | B | 0.33 | 0.260 | 0.233 | −0.028 |
| NOI2015 | 72.4 | 1081 | B | 0.67 | 0.457 | 0.238 | **−0.219** |
| NOI2015 | 77.3 | 1081 | B | 0.67 | 0.501 | 0.301 | **−0.200** |
| NOI2015 | 83.8 | 1081 | B | 0.67 | 0.531 | 0.401 | **−0.130** |
| GER2018 | 88.4 | 748 | U | 0.25 | 0.540 | 0.485 | −0.055 |
| GER2018 | 90.8 | 739 | U | 0.25 | 0.610 | 0.533 | −0.077 |
| GER2018 | 129.4 | 702 | U | 0.25 | 1.000 | 1.000 | −0.000 |

The residuals are now mostly **negative** — the model is *early*, not late, on these
points, the opposite of what the hard-coded values suggested. The four largest sit on the
hot Noirot discs at 1023–1081 K and on the one Gerczak point at 845 K, i.e. where the
temperature suppression of Eq. (7) is strongest.

### The two quantities only option 4 produces

| observable | N | RMSE_w | R²_w | plain RMSE | plain R² |
|---|---|---|---|---|---|
| mean misorientation Θ | 52 | 1.896 ° | +0.733 | 1.864 ° | +0.749 |
| subgrain radius r_n | 26 | 0.1216 µm | +0.017 | 0.1222 µm | −0.041 |

Per paper:

| observable | paper | N | weight share | RMSE_w | R²_w |
|---|---|---|---|---|---|
| Θ | Zacharie std UO₂ | 27 | 57.9 % | 2.137 ° | +0.679 |
| Θ | Zacharie Cr-doped | 11 | 16.2 % | 2.174 ° | +0.643 |
| Θ | Onofri (2025) | 14 | 25.9 % | 0.845 ° | — (all points at Θ = 0) |
| r_n | Zacharie std UO₂ | 14 | 58.7 % | 0.104 µm | +0.470 |
| r_n | Zacharie Cr-doped | 8 | 23.6 % | 0.134 µm | −1.413 |
| r_n | Gerczak (2018) | 4 | 17.7 % | 0.155 µm | −42.9 |

Raising the figure-digitized rank costs the radius: Gerczak goes from 3.9 % to 17.7 % of
the radius weight, and its rim points are the worst-predicted of the set, so R²_w on the
radius falls from +0.24 to +0.02. Θ improves (+0.70 → +0.73), because Onofri's fourteen
well-predicted Θ = 0 points go from 9.4 % to 25.9 %.

These are the scores of the **shipped set-C calibration** (k = 0.510, β = 26.68,
ρ_c = 5.74e15), which is fitted with exactly these weights.
...
`beta = 33.55` as the transformation rate and `rho_c` as a resolution length.
...
`calibrate.py` fits k, beta and rho_c of the Landau model on three data sets (default: **C**) — **A**
