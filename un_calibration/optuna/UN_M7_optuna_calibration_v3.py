#!/usr/bin/env python3
"""
UN_M7_optuna_calibration_v3.py
...
Purpose of v3:
  ...
  - add only a weak high-temperature radius guard, not an exact Rizk-radius fit.

Recommended first use:
  python UN_M7_optuna_calibration_v3.py --family capture_only --dv-min 0.3 --dv-max 1.5 \
    --partition-weight 0.8 --pressure-weight 0.7 --pressure-free-factor 2 \
    --rizk-prior-weight 0.2 --n-trials 120 --n-top-final 4 --no-plots

Then rerun/continue same folder with plots:
  python UN_M7_optuna_calibration_v3.py --family capture_only --dv-min 0.3 --dv-max 1.5 \
    --partition-weight 0.8 --pressure-weight 0.7 --pressure-free-factor 2 \
    --rizk-prior-weight 0.2 --n-trials 0 --n-top-final 4
"""
...
# In v3 gas partition is the cleaner way to constrain bulk/dislocation/q_gb behaviour.
W_BULK_SHAPE = 0.0
...
def optuna_candidate_from_trial_v3(trial, family: str):
    vals = {
        # f_n is treated as a closure parameter of the single-size model, so the range is wider
        # than v2 and the prior on it is weak.
        "f_n": trial.suggest_float("f_n", 1.0e-9, 1.0e-6, log=True),
        "K_d": trial.suggest_float("K_d", 1.5e5, 1.0e6, log=True),
        "rho_d": trial.suggest_float("rho_d", 1.0e13, 6.0e13, log=True),
        ...
        "gb_scale": trial.suggest_float("gb_scale", 0.15, 5.0, log=True),
        "gd_scale": trial.suggest_float("gd_scale", 0.15, 6.0, log=True),
        ...
        "capture_scale": trial.suggest_float("capture_scale", 0.03, 5.0, log=True),
    }
    if family == "baseline":
        vals["capture_scale"] = 0.0
    return m.Candidate(label=f"{family}_v3_trial_{trial.number:05d}", **vals)


def rizk_prior_score_v3(cand) -> float:
...
def gas_partition_score_v3(cand, dt_h: float, n_modes: int) -> Tuple[float, float, Dict[str, float]]:
    ...
    anchors = [
        ...
        (1.1, 1200.0, 68.0, None, None, 8.0, "1p1_1200"),
        (1.1, 1500.0, 68.0, None, None, 10.0, "1p1_1500"),
        (1.1, 1600.0, 55.0, None, None, 12.0, "1p1_1600"),
        (1.1, 2000.0, None, 85.0, 15.0, 15.0, "1p1_2000"),
        (1.1, 2200.0, None, 90.0, 10.0, 18.0, "1p1_2200"),
        (3.2, 1200.0, 70.0, None, None, 8.0, "3p2_1200"),
        (3.2, 1500.0, 70.0, None, None, 10.0, "3p2_1500"),
        (3.2, 1800.0, 45.0, 35.0, None, 15.0, "3p2_1800"),
...
def highT_radius_guard_score_v3(cand, dt_h: float, n_modes: int) -> Tuple[float, Dict[str, float]]:
    # Weak guard only. Do not force exact Rizk radius at 2000 K because high-T coarsening/release is uncertain.
    ...
    r1800 = out1800["Rd_nm"]
    r2000 = out2000["Rd_nm"]
    ratio = r2000 / r1800 if r1800 > 0 else math.inf
    terms = [
        max(0.0, ratio - 2.5) / 2.5,
        max(0.0, r2000 - 900.0) / 900.0,
    ]
    return _rmse(terms), {"Rd_1p3_1800K": r1800, "Rd_1p3_2000K": r2000, "Rd_ratio_2000_over_1800": ratio}


def score_candidate_v3(cand, dt_h: float, n_modes: int, use_full_exp: bool = True):
    if use_full_exp:
    ...
    score_rizk_prior = rizk_prior_score_v3(cand)
    score_partition, score_qgb, part_diag = gas_partition_score_v3(cand, dt_h, n_modes)
    score_radius_guard, radius_diag = highT_radius_guard_score_v3(cand, dt_h, n_modes)
...
def enqueue_known_good_trials_v3(study):
    # Keep base anchors, then add Codex capture_only anchors.
    try:
        m.enqueue_known_good_trials(study)
    except Exception:
        pass
...
def main():
    parser = argparse.ArgumentParser(description="V3 Optuna calibration wrapper with gas partition/q_gb score.")
    ...
    parser.add_argument("--n-top-final", type=int, default=4)
    parser.add_argument("--full-exp-fast", action="store_true")
    parser.add_argument("--no-plots", action="store_true")
    parser.add_argument("--dv-fixed", type=float, default=None)
    parser.add_argument("--dv-min", type=float, default=0.30)
    parser.add_argument("--dv-max", type=float, default=1.50)
    ...
    parser.add_argument("--radius-guard-weight", type=float, default=0.04)
    args = parser.parse_args()

    global DV_FIXED, DV_MIN, DV_MAX, W_RIZK_PRIOR, W_PARTITION, W_QGB, W_RADIUS_GUARD
    ...
    m.optuna_candidate_from_trial = optuna_candidate_from_trial_v3
    m.score_candidate = score_candidate_v3
    m.enqueue_known_good_trials = enqueue_known_good_trials_v3
    ...
    if args.output_dir:
        ...
        out = os.path.join(
            ...
            args.family,
            f"{dv_tag}_pW{m.W_PRESSURE:g}_pF{m.PRESSURE_FREE_FACTOR:g}_part{W_PARTITION:g}_qgb{W_QGB:g}_prior{W_RIZK_PRIOR:g}",
        )

    print("#" * 120)
    print("V3 wrapper settings")
