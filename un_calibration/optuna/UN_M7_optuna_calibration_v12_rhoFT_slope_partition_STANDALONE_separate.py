#!/usr/bin/env python3
# Standalone v11: includes the v10c rhoFT2 dependency inline.
...
# label -> (rho_global_scale, rho_slope_scale). Needed because Candidate dataclass has no extra fields.
...
def rhoFT2_factor(T: float, cand) -> float:
    """Temperature multiplier with global scale and slope scale.

    For T <= 1300 K, this is the Ray-Blank linear trend with adjustable slope:
        f(T) = 1 + s_slope * (f_RB(T) - 1)

    For T > 1300 K, the same linear trend is extended up to T_sat, then kept
    constant. This avoids the unphysical freedom of lowering only the high-T
    plateau below the 1300 K value.
    """
    if not RHOFT2_ENABLED:
        return 1.0

    rho_global_scale, rho_slope_scale = _scales_for_candidate(cand)
    T = float(T)
    T_sat = max(float(RHO_TRANSITION_END_K), THIGH_RB + 1.0)
    T_eval = min(max(T, TLOW_RB), T_sat)

    # Extend the same Ray-Blank linear fit beyond 1300 K until T_sat.
    f_linear = rho10_fit(T_eval) / rho10_fit(TREF_RB)
    f_shape = 1.0 + rho_slope_scale * (f_linear - 1.0)

    return max(0.05, rho_global_scale * f_shape)
...
def optuna_candidate_from_trial_v10c(trial, family: str):
    ...
    rho_global_scale = _suggest_float_maybe_fixed(trial, "rho_global_scale", RHO_GLOBAL_MIN, RHO_GLOBAL_MAX, log=True)
    rho_slope_scale = _suggest_float_maybe_fixed(trial, "rho_slope_scale", RHO_SLOPE_MIN, RHO_SLOPE_MAX, log=True)
    ...
    RHO_SCALE_BY_LABEL[cand.label] = (rho_global_scale, rho_slope_scale)
    return cand
...
def candidate_from_score_row_v10c(row: Dict, label_prefix: str = "best_M7"):
    ...
    rho_global_scale = m.safe_float(row.get("rho_global_scale"), 1.0)
    rho_slope_scale = m.safe_float(row.get("rho_slope_scale"), 1.0)
    RHO_SCALE_BY_LABEL[cand.label] = (rho_global_scale, rho_slope_scale)
    return cand
...
def enqueue_known_good_trials_v10c(study):
    ...
    try:
        ...
        pass
    for k in [
        {"rho_global_scale": 1.0, "rho_slope_scale": 1.0, "K_d": 5.0e5},
        {"rho_global_scale": 1.0, "rho_slope_scale": 2.0, "K_d": 5.0e5},
        {"rho_global_scale": 1.3, "rho_slope_scale": 1.0, "K_d": 5.0e5},
    ]:
        try:
...
def score_candidate_v10c(cand, dt_h: float, n_modes: int, use_full_exp: bool = True):
    ...
    row["rho_global_scale"] = rho_global_scale
    row["rho_slope_scale"] = rho_slope_scale
...
def main():
    ...
    parser.add_argument("--rho-global-min", type=float, default=0.7)
    parser.add_argument("--rho-global-max", type=float, default=2.0)
    parser.add_argument("--rho-slope-min", type=float, default=0.5)
    parser.add_argument("--rho-slope-max", type=float, default=5.0)
    ...
    global RHOFT2_ENABLED, RHO_GLOBAL_MIN, RHO_GLOBAL_MAX, RHO_SLOPE_MIN, RHO_SLOPE_MAX, RHO_TRANSITION_END_K, RHO_FAB
    ...
    RHO_GLOBAL_MIN = args.rho_global_min
    RHO_GLOBAL_MAX = args.rho_global_max
    RHO_SLOPE_MIN = args.rho_slope_min
    RHO_SLOPE_MAX = args.rho_slope_max
    ...
    if args.output_dir:
        ...
        out = os.path.join(
            ...
            args.family,
            f"rhoGlobal{RHO_GLOBAL_MIN:g}-{RHO_GLOBAL_MAX:g}_rhoSlope{RHO_SLOPE_MIN:g}-{RHO_SLOPE_MAX:g}_Tsat{RHO_TRANSITION_END_K:g}"
    ...
    print(f"rho_global_scale range          = {RHO_GLOBAL_MIN:g} -- {RHO_GLOBAL_MAX:g}")
    print(f"rho_slope_scale range           = {RHO_SLOPE_MIN:g} -- {RHO_SLOPE_MAX:g}")
...
"""
UN_M7_optuna_calibration_v12_rhoFT_slope_partition.py

V12 diagnostic wrapper:
  - uses a Ray-Blank rho_d(F,T) law with two more physical factors:
      rho_global_scale : multiplies the whole dislocation-density correlation
      rho_slope_scale  : changes the T-slope; above 1300 K the same line is extended to T_sat and then kept constant
  - no independent high-T lowering factor is allowed: the high-T plateau is the continuation of the line, not rho(1300)*s_high.
...
def score_candidate_v11(cand, dt_h: float, n_modes: int, use_full_exp: bool = True):
    ...
    rho_global_scale, rho_slope_scale = v10c._scales_for_candidate(cand)
    result["rhoFT2_enabled"] = int(v10c.RHOFT2_ENABLED)
    result["rho_global_scale"] = rho_global_scale
    result["rho_slope_scale"] = rho_slope_scale
...
def main():
    ...
    v10c.RHO_GLOBAL_MIN = args.rho_global_min
    v10c.RHO_GLOBAL_MAX = args.rho_global_max
    v10c.RHO_SLOPE_MIN = args.rho_slope_min
    v10c.RHO_SLOPE_MAX = args.rho_slope_max
    ...
    if args.output_dir:
        ...
        out = os.path.join(
            ...
            args.family,
            f"rhoGlobal{v10c.RHO_GLOBAL_MIN:g}-{v10c.RHO_GLOBAL_MAX:g}_rhoSlope{v10c.RHO_SLOPE_MIN:g}-{v10c.RHO_SLOPE_MAX:g}_Tsat{v10c.RHO_TRANSITION_END_K:g}"
            f"_qgb{W_V11_QGB:g}_part{W_V11_PARTITION:g}_Rfig8{W_RIZK_FIG8_DISL_RADIUS:g}",
        )

    print("#" * 120)
    print("V12 rhoFT-slope + gas-to-grain-face + high-T dislocation plateau settings")
    print(f"family                         = {args.family}")
    print(f"rhoFT2 enabled                 = {v10c.RHOFT2_ENABLED}")
    print(f"rho_global_scale range          = {v10c.RHO_GLOBAL_MIN:g} -- {v10c.RHO_GLOBAL_MAX:g}")
    print(f"rho_slope_scale range           = {v10c.RHO_SLOPE_MIN:g} -- {v10c.RHO_SLOPE_MAX:g}")
