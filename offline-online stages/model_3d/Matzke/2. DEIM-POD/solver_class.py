...
class FiniteElementCylinderSolver(object):
    ...
    def assemble(self, RADIUS=1E-5, LENGTH=1E-3, F_RATE=3E+19, F_YIELD=0.24, H_SOURCE=7.27E-18, T_BC=2E+03, C_BC=0.0, ALPHA_T=5E-7, dt=1E+4):

        # Define reference sources intensity:
        self.sgroup_T = F_RATE * H_SOURCE * LENGTH**2 / ALPHA_T 
        self.source_C = F_RATE * F_YIELD 

        # Define heat and mass diffusion coefficiets:
        self.alpha_C = 5.0e-08 * np.exp(-40262.0 / (T_BC + self.sgroup_T * (1 - self.ZZ_FO**2) / 2))
