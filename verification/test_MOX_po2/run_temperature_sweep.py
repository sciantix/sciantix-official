...
def run_comparison() -> None:
    ...
    oc_databases = ["upuo-v21.TDB"]
    oc_status = detect_oc_status(oc_databases)
    if oc_status.available_for(oc_databases):
        subprocess.run(
        ...
        print(f"[WARN] Skipping CALPHAD comparison: {oc_status.reason_for(oc_databases)}")
