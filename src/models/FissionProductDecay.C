...
void Simulation::FissionProductDecay()
{
    ...
    for (auto& system : sciantix_system)  // qui tutti i FPs
    {
        ...
        {
            sciantix_variable[system.getFissionProductName() + " decayed"].setFinalValue(
                solver.Decay(sciantix_variable[system.getFissionProductName() + " decayed"].getInitialValue(),
