...
void Simulation::HighBurnupStructureFormation()
{
    ...
    double coefficient =
        model["High-burnup structure formation"].getParameter().at(0) * pow(0.8814, - model["High-burnup structure formation"].getParameter().at(0)) *
        model["High-burnup structure formation"].getParameter().at(1) *
        pow(sciantix_variable["Effective burnup"].getFinalValue(), model["High-burnup structure formation"].getParameter().at(0) - 1.);
