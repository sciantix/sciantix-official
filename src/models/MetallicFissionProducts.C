...
void Simulation::MetallicFissionProducts()
{
    ...
    const double k_intra          = 1 * scaling_factors["MFP precipitation rate intragranular"].getValue();
    const double k_gb             = 1 * scaling_factors["MFP precipitation rate grain boundary"].getValue();
    ...
    const double dG_nucleation =
        1 * scaling_factors["MFP nucleation energy barrier"].getValue();  // energy barrier (eV)
    ...
    double       N_candidate_intra =
        solver.Decay(N_iniziale_intra,
                     k_nucleazione,
                     k_nucleazione * (f_dislocation * dislocation_sites + f_bubbles * bubble_sites_intra),
                     physics_variable["Time step"].getFinalValue());

    double N_candidate_inter = solver.Decay(N_iniziale_inter,
                                            k_nucleazione,
