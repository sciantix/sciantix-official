...
void Simulation::MetallicFissionProducts()
{
    ...
    // CONCENTRAZIONE LIBERA IN MATRICE Cm matrix
        ...
        double cm_matrix_new = (cm_matrix_old + dt * (y * fission_rate
                                + k_res * (cm_prec_intra_old
                                + cm_prec_gb_old))) / (1.0 + dt * (k_intra + k_gb));
    ...
    // Cm PRECIPIATTA INTRAGRANO
        ...
        double cm_prec_intra_new = (cm_prec_intra_old + dt * k_intra * cm_matrix_new)
                                   / (1.0 + dt * k_res);
    ...
    // Cm PRECIPITATA A GB 
        ...
        double cm_prec_gb_new    = (cm_prec_gb_old + dt * k_gb * cm_matrix_new)
                                    / (1.0 + dt * k_res);
