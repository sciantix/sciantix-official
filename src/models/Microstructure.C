...
void Simulation::Microstructure()
{
    ...
    double       lattice_parameter =
        (lattice_parameter_UO2 -
         1.2 * 1e-6 * (sciantix_variable["Chromium content"].getFinalValue())) *
        1e-10;  // empirical correlation for the lattice parameter

    // routine to calculate the molar mass of Uranium
    double conv_fact = sciantix_variable["Fuel density"].getFinalValue() * avogadro_number * 10 *
                       0.8815 *
    ...
    const double Cr_content = sciantix_variable["Chromium content"].getFinalValue() * 1e-6 *
                              (molar_mass_Uranium + 2 * molar_mass_Oxygen) /
                              (sciantix_variable["Chromium content"].getFinalValue() * 1e-6 *
                                   (molar_mass_Uranium - molar_mass_Chromium) +
                               molar_mass_Uranium);  //(mol/m3)

    double theoretical_density = (4 *
                                  ((1 - Cr_content) * molar_mass_Uranium +
                                   Cr_content * molar_mass_Chromium + 2 * molar_mass_Oxygen) /
                                  (avogadro_number * pow(lattice_parameter * 1e2, 3))) *
                                 1e3;  // (kg/m3)
