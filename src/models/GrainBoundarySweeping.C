...
void Simulation::GrainBoundarySweeping()
{
    ...
    switch (int(input_variable["iGrainBoundarySweeping"].getValue()))
    {
        ...
        {
            /**
             * @brief iGrainBoundarySweeping = 1 considers the fraction of grain swept volume (dV/V
             * = 3 dr / r). Then the fraction of intra-granular gas concentration swept is dC / C =
             * - 3 dr /
             * r
             *
             */
            ...
            parameter.push_back(3 * sciantix_variable["Grain radius"].getIncrement() /
        ...
        case 4:
        {
            for (int i = 0; i < n_modes; ++i)
            {
                modes_initial_conditions_NUS[7 * 40 + i] =
                    solver.Decay(modes_initial_conditions_NUS[7 * 40 + i],
                                 1.0,
                                 0.0,
                                 model["Grain-boundary sweeping"].getParameter().at(0));

                modes_initial_conditions_NUS[8 * 40 + i] =
                    solver.Decay(modes_initial_conditions_NUS[8 * 40 + i],
                                 1.0,
                                 0.0,
                                 model["Grain-boundary sweeping"].getParameter().at(0));
            }
