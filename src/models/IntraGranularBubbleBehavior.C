...
namespace
{
    ...
    constexpr double un_rho_fab = 3.0e13;  // UN AD URANIUMNITRIDE: Rizk nominal constant rho_d = 3.0e13; used/calibrated dynamic rho_d floor = 3.0e13.
    constexpr double un_rho_amp = 7.5e14;  // UN AD URANIUMNITRIDE: Rizk nominal dynamic amplitude = 0.0; used/calibrated = 7.5e14.
    constexpr double un_rho_scale = 0.10;  // UN AD URANIUMNITRIDE: Rizk nominal dynamic scale = 0.0; used/calibrated = 0.10.
    constexpr double un_rho_fc_percent = 3.0;  // UN AD URANIUMNITRIDE: Rizk nominal burnup scale = not specified; used/calibrated = 3.0 FIMA percent.
    constexpr double un_rho_t_half = 1550.0;  // UN AD URANIUMNITRIDE: Rizk nominal temperature half-point = not specified; used/calibrated = 1550 K.
    constexpr double un_rho_width = 120.0;  // UN AD URANIUMNITRIDE: Rizk nominal temperature width = not specified; used/calibrated = 120 K.
    constexpr double un_rho_f_min = 0.08;  // UN AD URANIUMNITRIDE: Rizk nominal high-temperature floor factor = not specified; used/calibrated = 0.08.
    constexpr double un_rho_cap = 4.0e15;  // UN AD URANIUMNITRIDE: Rizk nominal dynamic cap = not specified; used/calibrated = 4.0e15.
    constexpr double un_dv_dislocation_scale = 10.0;  // UN AD URANIUMNITRIDE: Rizk nominal dislocation vacancy scale = 1.0; used/calibrated = 10.0.
    constexpr double un_gf_ngf_areal_0 = 2.0e13;  // UN AD URANIUMNITRIDE: Rizk nominal grain-face areal density = not specified; used/calibrated = 2.0e13 m^-2.
    constexpr double un_gf_delta_gb = 4.0e-10;  // UN AD URANIUMNITRIDE: Rizk nominal grain-boundary thickness = not specified; used/calibrated = 4.0e-10 m.
    constexpr double un_gf_fc_sat = 0.5;  // UN AD URANIUMNITRIDE: Rizk nominal grain-face saturation coverage = not specified; used/calibrated = 0.5.
    constexpr double un_gf_theta = 1.0297442586766543;  // UN AD URANIUMNITRIDE: Rizk nominal semidihedral angle = not specified; used/calibrated = 1.0297442586766543 rad.
    constexpr double un_gf_r0 = 2.42e-10;  // UN AD URANIUMNITRIDE: Rizk nominal grain-face seed radius = not specified; used/calibrated = 2.42e-10 m.
    constexpr double un_gf_dv_gb_multiplier = 1.0e6;  // UN AD URANIUMNITRIDE: Rizk nominal grain-boundary vacancy multiplier = not specified; used/calibrated = 1.0e6.
    constexpr double un_first_pressure_factor = 1.0 / 3.0;  // UN AD URANIUMNITRIDE: Rizk nominal initial pressure factor = not specified; used/calibrated = 1.0/3.0.
    ...
    double un_vacancy_diffusivity(const double temperature, const double fission_rate, const int option)
    {
        ...
        if (option == 0)
        {
            const double d1 = 1.35e-2 * std::exp(-5.66 / kbt);  // UN AD URANIUMNITRIDE: Rizk nominal Dv1 = 1.35e-2*exp(-5.66/kT); used/calibrated = old notebook option.
            const double d2 = std::sqrt(std::max(fission_rate, 0.0)) * 4.6304523933553033e-29 *
                              safe_exp(-0.62 / kbt - 0.04 / (kbt * kbt));
            // UN AD URANIUMNITRIDE: Rizk nominal for U vacancies A20 = 1.32e-19, B21 = -0.62, B22 = -0.04; used/calibrated old vacancy refit A20 = 4.6304523933553033e-29, B21 = -0.62, B22 = -0.04.
            return d1 + d2;
        }
        if (option == 1)
        {
            const double d1 = 1.35e-2 * std::exp(-5.66 / kbt);  // UN AD URANIUMNITRIDE: Rizk nominal Dv1 = 1.35e-2*exp(-5.66/kT); used/calibrated = Rizk-2025 A20-only refit option.
            const double d2 = std::sqrt(std::max(fission_rate, 0.0)) * 1.386341579723e-28 *
                              safe_exp(-0.62 / kbt - 0.04 / (kbt * kbt));
            // UN AD URANIUMNITRIDE: Rizk nominal for U vacancies A20 = 1.32e-19, B21 = -0.62, B22 = -0.04; used/calibrated A20-only refit A20 = 1.386341579723e-28, B21 = -0.62, B22 = -0.04.
            return d1 + d2;
        }
        ...
        // UN AD URANIUMNITRIDE: Rizk nominal for U vacancies A20 = 1.32e-19, B21 = -0.62, B22 = -0.04; used/calibrated notebook-8 full refit A20 = 7.805188680989e-28, B21 = 9.932675113163e-01, B22 = 2.082395503235e-02.
        return d1 + d2;
    }

    double un_dynamic_rho(const double temperature, const double burnup_percent, const int option, const double constant_rho)
    {
        if (option == 0)
            return std::max(constant_rho, 1.0e10);

        const double burnup_part = 1.0 - std::exp(-std::max(burnup_percent, 0.0) / un_rho_fc_percent);
        const double ft = un_rho_f_min + (1.0 - un_rho_f_min) /
                                               (1.0 + std::exp((temperature - un_rho_t_half) / un_rho_width));
    ...
    {
        return volume > 0.0 ? std::pow(3.0 * volume / (4.0 * un_pi * std::max(gf_lens_factor(), 1.0e-30)), 1.0 / 3.0) : 0.0;
    }
...
void Simulation::IntraGranularBubbleBehavior()
{
    ...
    if (int(input_variable["iIntraGranularBubbleBehavior"].getValue()) == 5)
    {
        ...
        const double dmb_dt = dt > 0.0 ? (mb - positive(sciantix_variable["Xe in intragranular bubbles"].getInitialValue())) / dt : 0.0;
        const double dmd_dt = dt > 0.0 ? (md - positive(sciantix_variable["Xe in dislocation bubbles"].getInitialValue())) / dt : 0.0;
        const double dnvb_dt = dt > 0.0 ? (nvb - nvb_old) / dt : 0.0;
        const double dnvd_dt = dt > 0.0 ? (nvd - nvd_old) / dt : 0.0;

        const double Vb_growth = Nb > 0.0 ? std::max(Vb_old + dt * (un_omega_fg / Nb * dmb_dt + omega_matrix() / Nb * dnvb_dt), 0.0) : 0.0;
        ...
        const double qgb_target =
            std::max(sciantix_variable["Xe produced"].getFinalValue() - sciantix_variable["Xe decayed"].getFinalValue() -
                         (c + mb + md),
                     0.0);
        ...
        if (intergranular_option != 0 && dt > 0.0)
        {
            ...
            const double p_gf = ng > 0.0 && nv > 0.0 ? boltzmann_constant * temperature * ng / (nv * omega_matrix()) : 0.0;
            ...
            if (Fc >= un_gf_fc_sat && Ngf_areal > 0.0 && Ngf_vol > 0.0)
            {
                ...
                const double R_sat = std::sqrt(A_sat / (un_pi * std::max(std::pow(std::sin(un_gf_theta), 2.0), 1.0e-30)));
                const double V_sat = gf_volume_from_radius(R_sat);
                double p_ref = ng > 0.0 && nv > 0.0 ? boltzmann_constant * temperature * ng / (nv * omega_matrix()) : 0.0;
                if (!std::isfinite(p_ref) || p_ref <= 0.0)
                    p_ref = pressure_equilibrium(R_sat, hydrostatic_stress_pa);
                p_ref = std::max(p_ref, 1.0e-30);
                const double ng_allowed = std::max(V_sat / (un_omega_fg + boltzmann_constant * temperature / p_ref), 0.0);
        ...
        sciantix_variable["UN fission gas release"].setFinalValue(
            sciantix_variable["Xe produced"].getFinalValue() > 0.0 ? q_rel / sciantix_variable["Xe produced"].getFinalValue() : 0.0);
