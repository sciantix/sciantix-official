...
double coarseningBubblePressure(const double atoms_per_bubble,
...
void Simulation::IntraGranularBubbleBehavior()
{
    ...
    switch (int(input_variable["iIntraGranularBubbleBehavior"].getValue()))
    {
        ...
        case 4:
        {
            ...
            // COARSENING: bulk bubbles use the same nucleation structure as Pizzocri et al. with size-dependent re-solution.
            parameter.push_back(coarseningResolutionRate(
    ...
    if (int(input_variable["iIntraGranularBubbleBehavior"].getValue()) == 4)
    {
        ...
        const double dislocation_density             = 4.0e13;  // COARSENING: m/m3, Barani et al. Table 1.
        ...
        if (dislocation_bubble_concentration <= 0.0 && fission_rate > 0.0)
            dislocation_bubble_concentration = initial_dislocation_bubbles;
        ...
        for (auto& system : sciantix_system)
        {
            if (system.getGas().getDecayRate() == 0.0 && system.getRestructuredMatrix() == 0)
            {
                total_gas_in_bubbles +=
                    coarseningPositive(sciantix_variable[system.getGasName() + " in intragranular bubbles"].getFinalValue());
                total_gas_in_solution +=
                    coarseningPositive(sciantix_variable[system.getGasName() + " in intragranular solution"].getFinalValue());
            }
        }
        ...
        const double bulk_trapping_rate =
            4.0 * M_PI * gas_diffusivity * bulk_bubble_radius * bulk_bubble_concentration;
        const double dislocation_bubble_trapping_rate =
            4.0 * M_PI * gas_diffusivity * std::max(dislocation_bubble_radius_i, bulk_bubble_radius) *
            dislocation_bubble_concentration;
        const double dislocation_line_trapping_rate =
        ...
        if (total_gas_in_bubbles > 0.0 && total_trapping_rate > 0.0 && time_step > 0.0)
        {
            ...
            const double relaxation =
                1.0 - exp(-(total_trapping_rate + coarseningResolutionRate(dislocation_bubble_radius_i, fission_rate)) *
                          time_step);
        ...
        double coarsened_bubble_volume =
            coarsened_atoms_per_bubble * gas_atom_volume_in_bulk_bubbles +
        ...
        if (coarsened_bubble_radius > 0.0)
            equilibrium_pressure =
                2.0 * uo2_gas_surface_energy / coarsened_bubble_radius -
                history_variable["Hydrostatic stress"].getFinalValue() * 1.0e6;

        if (time_step > 0.0 && dislocation_bubble_concentration > 0.0 && coarsened_bubble_pressure > equilibrium_pressure)
        {
            ...
            const double z_denominator =
                std::max(-pow(psi, 6.0) + 5.0 * pow(psi, 2.0) - 9.0 * psi + 5.0, 1.0e-12);
        ...
        sciantix_variable["Intragranular coarsened bubble concentration"].setFinalValue(dislocation_bubble_concentration);
        ...
        sciantix_variable["Intragranular coarsened bubble equilibrium pressure"].setFinalValue(equilibrium_pressure / 1.0e6);
