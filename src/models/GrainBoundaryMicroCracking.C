...
void Simulation::GrainBoundaryMicroCracking()
{
    ...
    switch (int(input_variable["iGrainBoundaryMicroCracking"].getValue()))
    {
        ...
        case 1:
        {
            ...
            const double microcracking_parameter =
                (transient_type / span) * exp(arg) * pow((exponent * exp(arg) + 1), -1. / exponent - 1.);  // dm/dT
        ...
        case 2:
        {
            ...
            double equilibriumpressure = 2.0 * fuel_.getSurfaceTension() * (1 - cos(fuel_.getSemidihedralAngle())) /
            ...
            if (sciantix_variable["Intergranular fractional coverage"].getFinalValue() > 0.01)
                h = pow(
                    1.0 - 1.0 / (M_PI * Y *
                                 (2.0 * sqrt(pow(sciantix_variable["Intergranular fractional coverage"].getFinalValue(),
                                                 -0.5) -
                                             1.0))),
                    -1.0);
            ...
            double fracture_stress = K_IC * 1e6 /
            ...
            {
                atompervacancyf = sciantix_variable["Intergranular atoms per bubble"].getFinalValue() /
            ...
            {
                microcracking_parameter = (bubble_pressure - equilibriumpressure) *
        ...
        default:
            ErrorMessages::Switch(
                __FILE__, "iGrainBoundaryMicroCracking", int(input_variable["iGrainBoundaryMicroCracking"].getValue()));
            break;
    }
    ...
    if (input_variable["iReleaseMode"].getValue() == 0)
    {
        ...
        sciantix_variable["Intergranular saturation fractional coverage"].setFinalValue(
            solver.Decay(sciantix_variable["Intergranular saturation fractional coverage"].getInitialValue(),
                         1.0,
                         0.0,
    ...
    if (input_variable["iReleaseMode"].getValue() == 0)
    {
        ...
        sciantix_variable["Intergranular saturation fractional coverage"].setFinalValue(
            solver.Decay(sciantix_variable["Intergranular saturation fractional coverage"].getFinalValue(),
                         -model["Grain-boundary micro-cracking"].getParameter().at(2) *
    ...
    if (input_variable["iReleaseMode"].getValue() == 0)
    {
        ...
        if (sciantix_variable["Intergranular fractional coverage"].getInitialValue() > 0.0)
            similarity_ratio = sqrt(sciantix_variable["Intergranular fractional coverage"].getFinalValue() /
                                    sciantix_variable["Intergranular fractional coverage"].getInitialValue());
        else
