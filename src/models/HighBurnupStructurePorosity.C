...
void Simulation::HighBurnupStructurePorosity()
{
    ...
    if ((int)input_variable["iFuelMatrix"].getValue() != 2) // spengo HBS per il MOX
    {
    ...
    // average (at/m^3) of gas atoms in HBS pores
        ...
        sciantix_variable["Xe in HBS pores - variance"].setFinalValue(
            ...
            matrices[fuel_matrix_name].getPoreTrappingRate() * sciantix_variable["HBS pore density"].getFinalValue() - matrices[fuel_matrix_name].getPoreResolutionRate() * sciantix_variable["HBS pore density"].getFinalValue() + matrices[fuel_matrix_name].getPoreNucleationRate() * pow((sciantix_variable["Xe atoms per HBS pore"].getFinalValue() - 2.0), 2.0),
            physics_variable["Time step"].getFinalValue()
            )
        );
    ...
    {
        // Se siamo in MOX: nessuna HBS
        sciantix_variable["HBS pore density"].setFinalValue(0.0);
        sciantix_variable["HBS pore volume"].setFinalValue(0.0);
        sciantix_variable["HBS pore radius"].setFinalValue(0.0);
        sciantix_variable["Xe in HBS pores"].setFinalValue(0.0);
        sciantix_variable["Xe atoms per HBS pore"].setFinalValue(0.0);
        sciantix_variable["Xe in HBS pores - variance"].setFinalValue(0.0);
        sciantix_variable["Xe atoms per HBS pore - variance"].setFinalValue(0.0);
    }
}