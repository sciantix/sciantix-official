...
void Simulation::CallThermochemistryModule(std::string location, SciantixArray<SciantixVariable>& sciantix_variable)
{
    ...
    const ThermochemistryPhaseSettings& location_settings = (location == "matrix")
                                                                ? Sciantix_thermochemistry_settings.matrix
                                                                : Sciantix_thermochemistry_settings.fission_products;
    ...
    const bool oxygen_potential_constraint =
        (selected_elements.count("O") > 0 && selected_elements.count("U") == 0 && selected_elements.count("Pu") == 0);
    ...
    for (const auto& solve_attempt : solve_attempts)
    {
            ...
            fallback_oxygen_moles <= 0.0)
        {
            std::cout << "Warning: skipping " << OCUtilsCoupling::solveModeLabel(solve_attempt)
                      << " because no fallback O moles were captured from previous failed MU(O) attempts." << std::endl;
            continue;
        }

        if (!OCUtilsCoupling::writeOpenCalphadInput(input_file_path,
                                                    output_file_path,
                                                    state_file_path,
                                                    data_path,
                                                    pressure,
        ...
        {
            std::cout << "Warning: OpenCalphad attempt for location '" << location << "' with "
                      << OCUtilsCoupling::solveModeLabel(solve_attempt) << " failed or timed out. Retrying."
                      << std::endl;
            continue;
        }
        ...
        if (!OCUtilsCoupling::hasInvalidEquilibriumResult(raw_output))
        {
            ...
            std::cout << "Warning: OpenCalphad produced an equilibrium for location '" << location << "' using "
        ...
        if ((location == "matrix") && raw_output.find("C1_MO2") != std::string::npos)
        {
            ...
            std::cout << "Warning: OpenCalphad returned a stoichiometric C1_MO2 stabilization failure for "
                      << "location '" << location << "' using " << OCUtilsCoupling::solveModeLabel(solve_attempt)
                      << ". Deferring acceptance until all solve attempts are exhausted." << std::endl;
        }

        std::cout << "Warning: OpenCalphad returned an invalid equilibrium for location '" << location << "' using "
                  << OCUtilsCoupling::solveModeLabel(solve_attempt) << "." << std::endl;
            ...
            solve_attempt != OpenCalphadSolveMode::FixedOxygenMolesFromInvalidPotentialSolve)
        {
            ...
            if (OCUtilsCoupling::tryGetOxygenMolesFromOutput(output_file_path, active_elements, extracted_oxygen_moles))
            {
                fallback_oxygen_moles = extracted_oxygen_moles;
                std::cout << "Captured fallback N(O)=" << fallback_oxygen_moles
                          << " from invalid MU(O) output for location '" << location << "'." << std::endl;
            }
        }
    }
    ...
    if (!solved)
    {
        if (has_deferred_matrix_stoichiometric_solution)
        {
            ...
            std::cout << "Warning: all OpenCalphad solve attempts failed for location '" << location
                      << "'. Accepting deferred stoichiometric C1_MO2 stabilization result." << std::endl;
        }
        else
        {
            const std::vector<std::string> valid_elements(active_elements.begin(), active_elements.end());
    ...
    OCUtilsCoupling::updateThermochemistryVariablesFromOutput(
        output_data.solution_phases, location, total_input_content, thermochemistry_variable);

    if (location == "matrix")
    {
        OCUtilsCoupling::updateMatrixFromOutput(output_data, pressure, temperature, sciantix_variable, matrices);
        return;
    }
    ...
    if (location == "at grain boundary")
    {
        OCUtilsCoupling::updateGrainBoundaryFromOutput(
            output_data.solution_phases, selected_elements, total_input_content, sciantix_variable);
        return;
    }
