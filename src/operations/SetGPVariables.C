                             ...
                             double val1, double val2)
{
    ...
    double result =
        (1 - dx) * (1 - dy) * Q11 + dx * (1 - dy) * Q21 + (1 - dx) * dy * Q12 + dx * dy * Q22;

    return result;
}

void Simulation::setGPVariables(int Sciantix_options[], double Sciantix_history[],
                                double Sciantix_variables[])
{
    ...
    if (input_variable["iFissionGasDiffusivity"].getValue() == 90)
    {
        ...
        std::ifstream input_fileS1(
            TestPath + "GPregression/Results/StandardDeviation_XeDiffusionCoefficient.txt");
        std::vector<std::vector<double>> matrixUpdateXeDiffusionCoefficient =
            ReadValuesFromFile(input_fileU1);
        std::vector<std::vector<double>> matrixStdDevXeDiffusionCoefficient =
            ReadValuesFromFile(input_fileS1);
        ...
        double XeDiffusionCoefficientGP =
            bilinearInterpolation(matrixUpdateXeDiffusionCoefficient, TT_vector, FF_vector,
                                  pow(10, 4) / history_variable["Temperature"].getFinalValue(),
