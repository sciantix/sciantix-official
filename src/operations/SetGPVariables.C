                             ...
                             double                                  val2)
{
    ...
    double result = (1 - dx) * (1 - dy) * Q11 + dx * (1 - dy) * Q21 + (1 - dx) * dy * Q12 + dx * dy * Q22;

    return result;
}

void Simulation::setGPVariables(int Sciantix_options[], double Sciantix_history[], double Sciantix_variables[])
{
    // Loading of files generated by GPs for Xe Diffusion Coefficient
    if (input_variable["iFissionProductDiffusivity"].getValue() == 90)
    {
        ...
        double XeDiffusionCoefficientGP =
            bilinearInterpolation(matrixUpdateXeDiffusionCoefficient,
                                  TT_vector,
                                  FF_vector,
