...
namespace
{
    ...
    std::string normalizeElementCase(const std::string& token, const std::vector<std::string>& valid_elements)
    {
        ...
        while (i < token.size())
        {
            ...
            std::transform(
                candidate.begin(), candidate.end(), candidate.begin(), [](unsigned char c) { return std::toupper(c); });
            const auto it = valid_set.find(candidate);
            if (it != valid_set.end())
            {
                result += it->second;
...
namespace OCUtilsCoupling
{
    ...
    std::string buildSolveCommandBlock(OpenCalphadSolveMode mode, double pressure, double fixed_oxygen_moles)
    {
    ...
    {
        return fileExists(state_file_path) || fileExists(state_file_path + ".OCU") ||
    ...
    bool hasInvalidEquilibriumResult(const std::string& output_text)
    {
        return output_text.find("not a valid equilibrium as last calculation failed") != std::string::npos ||
               ...
               output_text.find("*** The results listed below may be inconsistent with the current conditions") !=
                   std::string::npos;
    }
    ...
    void dumpParsedOcOutput(const OCOutputData& output_data)
    {
        ...
        for (const auto& phase_entry : output_data.solution_phases)
        {
            ...
            std::cout << "  Phase " << phase_name << " : moles=" << phase_data.moles << ", volume=" << phase_data.volume
                      << std::endl;
            ...
            if (!phase_data.elements.empty())
            {
                ...
                {
                    std::cout << "      " << element_entry.first << " = " << element_entry.second << std::endl;
                }
            }
            ...
            if (!phase_data.species.empty())
            {
                ...
                for (const auto& species_entry : phase_data.species)
                {
                    ...
                    std::cout << "      " << species_name << " : moles=" << species_data.moles
                              << ", volume=" << species_data.volume << std::endl;
    ...
    std::vector<OpenCalphadInputComponent>
    buildOpenCalphadInputComponents(const std::set<std::string>&     selected_elements,
                                    ...
                                    double&                          total_content)
    {
        ...
        for (const auto& element_name : selected_elements)
        {
            ...
            {
                const double atoms_available = sciantix_variable[element_name + " produced"].getFinalValue() -
                               ...
                               double                           fixed_oxygen_moles)
    {
        ...
        const bool use_oxygen_potential = (selected_elements.count("O") > 0 && selected_elements.count("U") == 0 &&
                                           selected_elements.count("Pu") == 0);

        const double oxygen_potential_j_per_mol = sciantix_variable["Fuel oxygen potential"].getFinalValue() * 1.0e3 /
                                                  2.0;  // Convert from kJ/mol O2 to J/mol O
        ...
        input_file << "c e\n";
        if (use_oxygen_potential && solve_mode != OpenCalphadSolveMode::FixedOxygenMolesFromInvalidPotentialSolve)
        {
        ...
        {
            input_file << buildSolveCommandBlock(solve_mode, pressure, fixed_oxygen_moles);
        }
                            ...
                            int                timeout_seconds)
    {
        ...
        const std::string command = "timeout --signal=TERM " + std::to_string(timeout_seconds) + "s " + executable;
                                                  ...
                                                  SciantixArray<ThermochemistryVariable>&   thermochemistry_variable)
    {
        ...
        auto logThermochemistryVariable = [&](const std::string& variable_name)
        {
            ...
            std::cout << "[Thermochemistry] " << variable_name << " composition=" << composition_label
                      << " molar_mass=" << thermochemistry_variable[variable_name].getMolarMass() << " g/mol"
                      << std::endl;
        };
        ...
        for (const auto& phase_entry : solution_phases)
        {
            ...
            const std::map<std::string, double> liquid_phase_composition =
                is_single_phase_liquid ? computeNormalizedPhaseComposition(phase_data)
            ...
            {
                thermochemistry_variable[liquid_phase_variable_name].setFinalValue(phase_data.moles *
                ...
                if (!liquid_phase_composition.empty())
                {
                    ...
                    std::cout << "[ThermochemistryPhase] " << phase_name << " (" << location
                              << ") composition=" << phase_stream.str() << " molar_mass=" << phase_molar_mass
                              << " g/mol" << std::endl;
                }
            ...
            if (is_single_phase_liquid && !liquid_phase_composition.empty())
            {
                ...
                const double phase_molar_mass = computeCompositionMolarMass(liquid_phase_composition);
                std::cout << "[ThermochemistryPhase] " << phase_name << " (" << location
                          << ") composition=" << phase_stream.str() << " molar_mass=" << phase_molar_mass << " g/mol"
                          << std::endl;
            }
        }
    }
