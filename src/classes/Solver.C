...
{   //Manufactured solution: C(r,t) = (a^2-r^2)*exp(0.005*t)
    ...
    for (n = 0; n < parameter.at(0); n++)
    {
        ...
        source_rate = 2 * sqrt(2) * sqrt(M_PI) * (
            -6.0 * pow(-1,np1) * pow(parameter.at(2),4) * cos(time) / (pow(M_PI,3) * pow(np1,3))
            - 6.0 * pow(-1,np1) * pow(parameter.at(2),2) * sin(time) / (M_PI * np1)
        ...
        solution += projection_coeff * n_coeff * initial_condition[n] / ((4. / 3.) * M_PI *pow(parameter.at(2), 1.5) );
    }
