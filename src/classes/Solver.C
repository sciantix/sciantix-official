...
double Solver::BisectionKato(std::vector<double> parameter)
{
    ...
    double temperature_adjusted =
        temperature_actual + (0.16 * sigmoid_S) * (1773.0 - temperature_actual);  // equation 8.5 NEA2024 pag 207
    ...
    for (int i = 0; i < max_iter; i++)
    {
        ...
        double log_v2 =
            -5.0 * (0.5 * (68.8 + 131.3 * q_eff) / R - 0.5 * 515000.0 / (R * temperature_adjusted) - 0.25 * ln_po2);
        double log_v3 =
            -5.0 * (((1.0 / 3.0) * (std::log(2.0) + (153.5 - 96.5 * q_eff + 331.0 * std::pow(q_eff, 2)) / R -
                                    891000.0 / (R * temperature_adjusted))) -
                    (1.0 / 3.0) * ln_po2);
        double log_v4 = -5.0 * std::log(0.5 * q_eff);

        // Log-Sum-Exp
        double max_log = std::max({log_v1, log_v2, log_v3, log_v4});
        double sum_exp = std::exp(log_v1 - max_log) + std::exp(log_v2 - max_log) + std::exp(log_v3 - max_log) +
                         std::exp(log_v4 - max_log);
