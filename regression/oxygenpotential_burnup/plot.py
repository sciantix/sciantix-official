...
def read_output_curve(case_dir: Path, column: str) -> tuple[np.ndarray, np.ndarray]:
    rows = read_table(case_dir / "output.txt")
    om = np.array([float(row["O/M ratio (/)"]) for row in rows], dtype=float)
    mu = np.array([float(row[column]) for row in rows], dtype=float)

    # A potential of exactly 0 means "not computed at this step" (e.g. the
    # CALPHAD column before the first equilibrium), not a real 0 kJ/mol.
    computed = mu != 0.0
    om = om[computed]
    mu = mu[computed]
...
def read_output_curve_Kato(case_dir: Path) -> tuple[np.ndarray, np.ndarray]:
    return read_output_curve(case_dir, "Fuel oxygen potential - Kato (KJ/mol)")

def read_output_curve_OC(case_dir: Path) -> tuple[np.ndarray, np.ndarray]:
    return read_output_curve(case_dir, "Fuel oxygen potential - CALPHAD (KJ/mol)")
