...
def main():
    root = os.path.dirname(__file__)
    cornell_root = os.path.abspath(os.path.join(root, "..", "cornell"))
    ...
    # loop over all Cornell cases
    for name in sorted(os.listdir(cornell_root)):
        if not name.startswith("test_Cornell"):
            continue

        case = os.path.join(cornell_root, name)
