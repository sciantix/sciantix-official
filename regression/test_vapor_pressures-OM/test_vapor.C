...
void calculate_pressures(double T, double q, double om, std::ofstream &file) {
    ...
    // partial pressures calculation [atm] (Olander pag 158 tab 11.1)
    ...
    // Costanti per i metalli puri gassosi atomici calibrate sul potenziale chimico solido dell'Olander
    double K_U_pure  = std::exp(-34000.0 / T - 2.1);
    double K_Pu_pure = std::exp(-29500.0 / T - 3.1);

    // Calcolo delle pressioni parziali delle specie ossidate gassose
    ...
    // Calcolo lineare diretto basato sulla concentrazione delle specie ridotte nel reticolo solido
    double p_u  = 2.0 * K_U_pure * n_u2;
    double p_pu = 2.0 * K_Pu_pure * n_pu2;

    if (std::isnan(p_u)  || std::isinf(p_u)  || p_u < 1e-45)  p_u = 1e-45;
    if (std::isnan(p_pu) || std::isinf(p_pu) || p_pu < 1e-45) p_pu = 1e-45;

    file << T << "," << q << "," << om << "," 
         << p_o2 << "," << p_u << "," << p_pu << "," << p_uo << "," << p_uo2 << "," << p_uo3 << "," 
         << p_puo << "," << p_puo2 << "\n";
} 

int main() {
    std::ifstream input_file("input_history.txt");
    std::ofstream output_file("vapor_results.csv");

    output_file << "Time,T,q,OM,p_O2,p_U,p_Pu,p_UO,p_UO2,p_UO3,p_PuO,p_PuO2\n";

    double time, T, fiss_rate, stress, hydro_pressure;
    const double q_fix = 0.244;
    
    double om_start = 1.88;
    double om_end = 2.12;
    double total_time = 10000.0; 

    std::cout << "Lettura history e calcolo sensibilità O/M..." << std::endl;

    while (input_file >> time >> T >> fiss_rate >> stress >> hydro_pressure) {
        double om_current = om_start + (time / total_time) * (om_end - om_start);

        output_file << time << ",";
        calculate_pressures(T, q_fix, om_current, output_file);
    }
