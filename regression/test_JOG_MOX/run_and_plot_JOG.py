...
def plot_radial_profiles(
    ...
    if available_oxygen_columns:
        ...
        radial_colors = evenly_spaced_colors(len(radii_mm_array))
        for column_name, label, linestyle in available_oxygen_columns:
            for color, radius_mm, case_history in zip(radial_colors, radii_mm_array, output_histories):
                if column_name not in case_history or is_all_zero(case_history[column_name]):
                    continue
                axis.plot(
                    case_history[BURNUP_LABEL],
                    case_history[column_name],
                    color=color,
                    alpha=0.9,
                    label=f"R/Ro = {radius_mm*1e-3/PELLET_RADIUS_M:.1f}",
                )
