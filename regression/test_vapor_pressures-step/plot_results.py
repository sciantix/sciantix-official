...
print("\n--- Generating Step Analysis Plot (Time Response) ---")
...
try:
    ...
    ax1.plot(df['Time'], df['T'], color=color_t, linewidth=3, label='Temperature')
    ax1.tick_params(axis='y', labelcolor=color_t)
    ax1.grid(True, which="both", linestyle=':', alpha=0.5)

    # --- ASSE 2: Pressioni (Logaritmico) ---
    ax2 = ax1.twinx()
    ax2.set_ylabel('Partial Pressure (atm)', fontsize=12)

    # Nomi delle specie (Devono corrispondere al tuo C++)
    species_to_plot = {
        'p_O2': 'black',
        'p_UO2': 'green',
        'p_UO3': 'cyan',
    ...
    for sp, col in species_to_plot.items():
        if sp in df.columns:
            # Usiamo linestyle '--' per distinguere le pressioni dalla temperatura
            ax2.plot(df['Time'], df[sp], label=sp, color=col, linewidth=2, linestyle='--')

    ax2.set_yscale('log')
    
    # Titolo e Legenda
    plt.title(f'Vaporization Step Response: Pressure vs Time\n(q={val_q}, O/M={val_om_fix})', fontsize=14)
    ...
    print(f"Errore durante il plot: {e}")