...
int main() {
    ...
    // 2. Intestazione CSV (fondamentale includere Time per il Python)
    output_file << "Time,T,q,OM,p_O2,p_UO,p_UO2,p_UO3,p_PuO,p_PuO2\n";

    // Variabili per la lettura (5 colonne come da standard Sciantix)
    double time, T, fiss_rate, stress, hydro_pressure;
    
    // Parametri chimici che teniamo fissi per lo Step Test
    const double q_fix = 0.244;
    const double om_fix = 1.98;

    std::cout << "Esecuzione STEP TEST da input_history.txt..." << std::endl;

    // 3. Ciclo di lettura: legge il salto di temperatura dal file
    while (input_file >> time >> T >> fiss_rate >> stress >> hydro_pressure) {
        
        // Scriviamo il tempo nel CSV per l'asse X del grafico
        output_file << time << ",";
        
        // Calcoliamo le pressioni basandoci sulla T letta (che farà il salto)
    ...
    std::cout << "Calcolo completato. Risultati in 'vapor_results.csv'" << std::endl;
    return 0;
}