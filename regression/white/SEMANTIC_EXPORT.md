...
- `input.json`: structured machine-readable input.
- `output.json`: structured machine-readable output. It includes a `schema` field pointing to: `metadata/schema/output.schema.json`
- `output.jsonld`: JSON-LD export for semantic/RDF mapping. It includes a `schema` field pointing to: `metadata/schema/input.schema.json`
- `case_metadata.jsonld`: JSON-LD case-level metadata linking inputs, outputs, software, and sources.

Case-level model and software source informations are stored in:
