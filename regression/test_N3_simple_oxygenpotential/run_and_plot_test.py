...
def radial_integral_over_radius(profile: np.ndarray, radii_m_array: np.ndarray, r_max_m: float) -> np.ndarray:
    return np.trapezoid(profile * radii_m_array[:, np.newaxis], x=radii_m_array, axis=0) / r_max_m
...
def radial_volume_average(profile: np.ndarray, radii_m_array: np.ndarray) -> np.ndarray:
    ...
    shell_weights = (shell_edges[1:] ** 2 - shell_edges[:-1] ** 2) / (r_max_m ** 2)
...
def plot_case(
    ...
    jog_thickness_um = jog_fraction * (PELLET_RADIUS_M / 2.0) * 1.0e6
    ...
    axis.text(
        0.98,
        0.98,
        f"Thickness = JOG (/) * R/2,  R = {PELLET_RADIUS_M * 1.0e3:.3f} mm",
    ...
    ) * (PELLET_RADIUS_M / 2.0) * 1.0e6
    liquid_thickness_um = liquid_cs2moo4_volume_fraction * (PELLET_RADIUS_M / 2.0) * 1.0e6
    condensed_thickness_um = condensed_cs2moo4_volume_fraction * (PELLET_RADIUS_M / 2.0) * 1.0e6
...
def plot_radial_profiles(
    ...
    for axis, (variable, ylabel) in zip(axes, radial_summary_variables):
        ...
        axis.set_xlabel("Radial profile (-)")
        axis.set_ylabel(ylabel)
        axis.grid(True, alpha=0.3)
    ...
    if gb_radial_histories:
        ...
        for boundary, color in zip(cumulative_histories, gb_colors):
            axis.plot(reference_burnup, boundary, color=color, linewidth=0.8, alpha=0.95)
    axis.set_xlabel("Burnup (MWd/kgUO2)")
    axis.set_ylabel("Concentration at grain-boundary (mol/m3)")
    axis.grid(True, alpha=0.3)
    add_capped_legend(axis, loc="upper left", fontsize=8)
    fig.tight_layout()
    plot_path = PLOTS_DIR / "radial_profile_thermochemistry_fission_products_at_grain_boundary_burnup.png"
    fig.savefig(plot_path, bbox_inches="tight")
    plt.close(fig)
    saved_paths.append(plot_path)
    
    if "JOG (/)" in output_profiles:
        jog_thickness_over_time_m = radial_integral_over_radius(output_profiles["JOG (/)"], radii_m_array, r_max_m)
        ...
        cs = thermo_profiles["CS (liquid, at grain boundary) (mol/m3)"]
        o = thermo_profiles["O (liquid, at grain boundary) (mol/m3)"]
        mo = thermo_profiles["MO (liquid, at grain boundary) (mol/m3)"]
        ...
        liquid_cs2moo4_thickness_over_time_m = radial_integral_over_radius(
            liquid_cs2moo4_volume_fraction,
            radii_m_array,
            r_max_m,
        )
        ...
        if (
            ...
            condensed_cs2moo4_thickness_over_time_m = radial_integral_over_radius(
        ...
        total_thermochemistry_thickness_over_time_um = liquid_cs2moo4_thickness_over_time_um.copy()
        if condensed_cs2moo4_thickness_over_time_um is not None:
            total_thermochemistry_thickness_over_time_um += condensed_cs2moo4_thickness_over_time_um
        ...
        def burnup_to_fima(burnup_values: np.ndarray) -> np.ndarray:
            return np.interp(burnup_values, reference_burnup, reference_fima)

        fig, axis = plt.subplots()
        axis.plot(reference_burnup, jog_thickness_over_time_um, color="black",label="Total JOG thickness")
        axis.plot(reference_burnup, liquid_cs2moo4_thickness_over_time_um, color="#f97316", label="Liquid")
        if condensed_cs2moo4_thickness_over_time_um is not None:
            axis.plot(
                reference_burnup,
        ...
        axis.set_xlabel("Burnup (MWd/kgUO2)")
        axis.set_ylabel("JOG thickness (um)")
        axis.secondary_xaxis("top", functions=(burnup_to_fima, fima_to_burnup)).set_xlabel("FIMA (%)")
        axis.legend(loc="best")
        axis.grid(True, alpha=0.3)
        fig.tight_layout()
        plot_path = PLOTS_DIR / "jog_thickness_over_burnup.png"
        fig.savefig(plot_path, bbox_inches="tight")
        plt.close(fig)
        saved_paths.append(plot_path)
        ...
        if jog_condensed_thickness_over_time_um is not None and jog_liquid_thickness_over_time_um is not None:
            ...
            axis.stackplot(
                ...
                jog_liquid_thickness_over_time_um,
                labels=["Condensed contribution", "Liquid contribution"],
                colors=["#2563eb", "#f97316"],
                alpha=0.8,
            )
            ...
            fig.tight_layout()
            plot_path = PLOTS_DIR / "jog_contributions_over_burnup.png"
            fig.savefig(plot_path, bbox_inches="tight")
            plt.close(fig)
            saved_paths.append(plot_path)
...
def main() -> int:
    ...
    for case_dir in case_directories:
        plot_case(case_dir, saved_paths, gb_color_map, matrix_color_map)
