...
# --- 1. FUNZIONI TERMODINAMICHE (Benchmark Python - Brentq) ---
# Formule caricate dal tuo documento (Kato & Olander) [cite: 116-196]
...
def solve_for_po2(target_om, T_K, C_pu, R=8.314):
    ...
    try:
        sol_log = brentq(objective, -150, 40) 
        return 10**sol_log
...
for spec, color in species_colors.items():
    # LINEA = Sciantix (C++) [cite: 400-402]
    plt.plot(df['T'], df[spec], color=color, linestyle='--', linewidth=1.5, alpha=0.9)
    # PALLINI = Brentq (Python) - Plottiamo tutti i punti della rampa [cite: 393-395]
    plt.scatter(valid_temps, brentq_results[spec], color=color, marker='o', s=20, alpha=0.7, zorder=5)

plt.yscale('log')
plt.xlabel('Temperature (K)', fontsize=12, fontweight='bold')
plt.ylabel('Partial Pressure (atm)', fontsize=12, fontweight='bold')
plt.grid(True, which="both", linestyle=':', alpha=0.5)

# Doppia Legenda [cite: 407-413]
method_legend = [
    Line2D([0], [0], color='gray', linestyle='--', lw=2, label='Sciantix (C++)'),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=10, label='Brentq (Benchmark)')
]
l1 = plt.legend(handles=method_legend, loc='upper left', title="Numerical Method")
plt.gca().add_artist(l1)

species_legend = [Line2D([0], [0], color=c, lw=2, label=s.replace('p_', '')) for s, c in species_colors.items()]
plt.legend(handles=species_legend, loc='lower right', title="Gas Species", ncol=2)

plt.title(f'Ramp Validation: Partial Pressures vs Temperature\n(q={val_q}, O/M={val_om_fix})')
plt.tight_layout()
plt.show()
