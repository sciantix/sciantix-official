...
int main() {
    ...
    std::cout << "Esecuzione Rampa da input_history.txt..." << std::endl;

    // 2. Leggiamo la rampa dal file history
    while (input_file >> time >> T >> fiss_rate >> stress >> hydro_pressure) {
        
        // Scriviamo il tempo per tenere allineate le colonne
        output_file << time << ",";
        
        // Calcoliamo le pressioni (la funzione scriverà il resto della riga)
    ...
    std::cout << "Rampa completata correttamente!" << std::endl;
    return 0;
}