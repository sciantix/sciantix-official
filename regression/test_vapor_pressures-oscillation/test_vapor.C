...
// pressures calculation - RESTA INVARIATA
...
int main() {
    // 1. Apertura file di input (history) e output (results)
    std::ifstream input_file("input_history.txt");
    std::ofstream output_file("vapor_results.csv");

    if (!input_file.is_open()) {
        std::cerr << "Errore: impossibile aprire input_history.txt" << std::endl;
        return 1;
    }

    // Intestazione CSV (aggiunto Time per riferimento)
    output_file << "Time,T,q,OM,p_O2,p_UO,p_UO2,p_UO3,p_PuO,p_PuO2\n";

    // Variabili per leggere le 5 colonne del file history
    double time, T, fiss_rate, stress, hydro_pressure;
    
    // Parametri chimici fissi (come richiesto)
    const double q_fix = 0.244;
    const double om_fix = 1.98;

    std::cout << "Lettura dati da input_history.txt in corso..." << std::endl;

    // 2. Ciclo di lettura riga per riga
    while (input_file >> time >> T >> fiss_rate >> stress >> hydro_pressure) {
        
        // Scriviamo il tempo nel CSV prima dei calcoli
        output_file << time << ",";
        
        // Chiamata alla funzione originale usando T dal file e i parametri fissi
        calculate_pressures(T, q_fix, om_fix, output_file);
    }

    input_file.close();
    output_file.close();

    std::cout << "Calcolo completato. Risultati salvati in 'vapor_results.csv'" << std::endl;
