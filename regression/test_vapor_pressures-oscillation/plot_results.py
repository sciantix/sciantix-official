...
# =============================================================================
# OSCILLATION CASE ANALYSIS
# =============================================================================

# --- 1. DEFINISCI LE VARIABILI ---
val_q = 0.244      # Il tuo contenuto di Plutonio
val_om_fix = 1.98  # Il tuo valore di O/M fisso

# --- 2. USA LA F-STRING CORRETTA ---
# Nota la 'f' prima dell'apertura delle virgolette
plt.title(f'Temporal Evolution: Vaporization Response to T-Oscillation\n(q={val_q}, O/M={val_om_fix})', fontsize=14)

print("\n--- Generating Oscillation Analysis Plots ---")

# 1. Caricamento dati dal CSV generato dal C++
df = pd.read_csv("vapor_results.csv")
df.columns = df.columns.str.strip() # Rimuove spazi bianchi che il C++ potrebbe aver aggiunto

# Creazione del grafico con doppio asse Y
fig, ax1 = plt.subplots(figsize=(12, 7))

# --- ASSE 1: Temperatura (Lineare) ---
color_temp = 'tab:red'
ax1.set_xlabel('Time (s)', fontsize=12)
ax1.set_ylabel('Temperature (K)', color=color_temp, fontsize=12)
ax1.plot(df['Time'], df['T'], color=color_temp, linewidth=3, alpha=0.8, label='Temperature')
ax1.tick_params(axis='y', labelcolor=color_temp)
ax1.grid(True, which="both", linestyle='--', alpha=0.5)

# --- ASSE 2: Pressioni Parziali (Logaritmica) ---
ax2 = ax1.twinx() 
ax2.set_ylabel('Partial Pressure (atm)', fontsize=12)

# Definiamo le specie da plottare e i loro colori (usando il tuo dizionario precedente)
species_to_plot = {
    'p_O2': 'black',
    'p_UO2': 'green',
    'p_UO3': 'cyan',
...
for species, color in species_to_plot.items():
    style = '--' if 'Pu' in species or 'O2' in species else '-'
    ax2.plot(df['Time'], df[species], color=color, linestyle=style, linewidth=2, label=species)

ax2.set_yscale('log')
# Impostiamo un limite sensato per le pressioni basandoci sui tuoi plot precedenti
ax2.set_ylim(df[list(species_to_plot.keys())].min().min() * 0.1, 
             df[list(species_to_plot.keys())].max().max() * 10)
...
# --- GRAFICO CONCENTRAZIONI NEL TEMPO ---
fig, ax3 = plt.subplots(figsize=(12, 6))

# Plottiamo le concentrazioni di Uranio e Plutonio nel tempo
ax3.plot(df['Time'], df['p_UO2']/4, label='$n_{U4+}$ (approx)', color='blue') # Esempio di ricalcolo inverso
ax3.set_xlabel('Time (s)')
ax3.set_ylabel('Relative Evolution')
ax3.set_title('Evolution of Species during Oscillation')
ax3.legend()
plt.grid(True, alpha=0.3)
plt.show()

# =============================================================================
# VERIFICA PUNTUALE: C++ vs Python durante l'oscillazione
# =============================================================================
# Questo serve a dimostrare che anche durante l'onda, il C++ non perde precisione

errors_osc = {k: [] for k in species_to_plot.keys()}
times = df['Time'].values

for index, row in df.iterrows():
    T_val = row['T']
    po2_py = solve_for_po2(val_om_fix, T_val, val_q)
    if po2_py:
        concs_py = solve_species_concentrations(po2_py, T_val, val_q)
        press_py = solve_partial_pressures(T_val, concs_py)
        
        for sp in species_to_plot.keys():
            # Errore relativo
            err = abs(row[sp] - press_py[sp]) / press_py[sp]
            errors_osc[sp].append(err)

plt.figure(figsize=(10, 5))
for sp, color in species_to_plot.items():
    plt.plot(times, errors_osc[sp], label=f'Err {sp}', color=color)

plt.yscale('log')
plt.xlabel('Time (s)')
plt.ylabel('Relative Error $|P_{cpp} - P_{py}| / P_{py}$')
plt.title('Numerical Stability Check during Oscillation')
