...
# Brentq calculus
# from test_vapor_pressures/plot_results.py [cite: 116-127, 137-196]
...
# Temperatura (Asse Sinistro) [cite: 270-275]
...
for spec, color in species_colors.items():
    # line Sciantix (C++) [cite: 400-402]
    ax2.plot(df['Time'], df[spec], color=color, linestyle='--', linewidth=1.5, alpha=0.9)
    # points Brentq (Benchmark)
    ax2.scatter(valid_times, brentq_results[spec], color=color, marker='o', s=20, alpha=0.7, zorder=5)
...
l1 = ax2.legend(handles=method_legend, loc='upper left', title="Numerical Method", framealpha=0.9)
ax2.add_artist(l1)

# dx
species_legend = [Line2D([0], [0], color=c, lw=2, label=s.replace('p_', '')) for s, c in species_colors.items()]
ax2.legend(handles=species_legend, loc='upper right', title="Gas Species", ncol=2, framealpha=0.9)

plt.title(f'Verification: Oscillation Response Analysis \n(q={val_q}, O/M={val_om_fix})')
plt.tight_layout()
plt.show()
