...
print('Burnup (MWd/kgMOX) = ', data['With Thermochemistry']['Burnup (MWd/kgMOX)'].max()) 
...
for isotope, properties in isotope_data.items():
    ...
    plt.plot(
        ...
        additional_data[element]['Normalized release (/)'],
        marker='o',
        linestyle='--',
        linewidth=0.5,
        color=colors[list(isotope_data.keys()).index(isotope)],
...
plt.xlim([750, 2773])
plt.ylim([0,1])
plt.title('Comparison with experimental data, Colle (2023)')

handles, labels = plt.gca().get_legend_handles_labels()
legend_markers = [plt.Line2D([0], [0], marker='o', linestyle='--', color='black', label='Colle (2023)' )]
...
legend_colors2 = [plt.Line2D([0], [0], marker='o', linestyle='--', color='grey', label=f'Experimental MOX')]
