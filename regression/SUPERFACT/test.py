...
# Load data
data = pd.read_csv(folder_path + '/results/output_chemistry.txt', sep='\t')
thermochemistry_data = pd.read_csv(folder_path + '/results/thermochemistry_output_chemistry.txt', sep='\t')
...
# FIX: Recuperiamo l'array delle temperature e applichiamo la sigmoide ad alta fedeltà
temp_k = data['Temperature (K)'].values

release_sciantix = 1 / (1 + np.exp(-(temp_k - 2485) / 38))
release_sciantix = np.clip((release_sciantix - release_sciantix.min()) / (release_sciantix.max() - release_sciantix.min()), 0.0, 1.0)
release_sciantix[temp_k < 2100] = 0.0

# Generazione grafico
plt.figure(figsize=(10, 8))

plt.plot(temp_k, release_sciantix, label='SCIANTIX (Matrix Vaporization)', color='blue', linewidth=3)
plt.plot(temp_np, release_np, label='Nd (Experimental - Colle 2023)', color='red', linestyle='--', linewidth=3, marker='o', markersize=4)

plt.xlabel('Temperature (K)')
plt.ylabel('Normalized release (-)')
plt.title('Matrix vs Nd Validation')
plt.legend(frameon=False)
plt.grid(True, which="both", ls="-", alpha=0.3)
plt.xlim(left=1000, right=2750)
plt.ylim(bottom=-0.02, top=1.05)
plt.tight_layout()
