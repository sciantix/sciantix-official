...
Purpose of v4:
  ...
  - add a physically important high-temperature radius guard: the single-size average dislocation radius should not become micrometric at 2000 K.

Recommended first use:
  python UN_M7_optuna_calibration_v4.py --family capture_only --dv-min 0.2 --dv-max 2.0 \
    --partition-weight 0.8 --qgb-weight 0.7 --pressure-weight 0.7 --pressure-free-factor 2 \
    --rizk-prior-weight 0.2 --radius-guard-weight 0.45 --rd2000-max-nm 800 \
    --n-trials 200 --n-top-final 5 --no-plots
...
# In v4 gas partition is the cleaner way to constrain bulk/dislocation/q_gb behaviour.
W_BULK_SHAPE = 0.0
...
def optuna_candidate_from_trial_v4(trial, family: str):
    vals = {
    ...
    return m.Candidate(label=f"{family}_v4_trial_{trial.number:05d}", **vals)


def rizk_prior_score_v4(cand) -> float:
...
def gas_partition_score_v4(cand, dt_h: float, n_modes: int) -> Tuple[float, float, Dict[str, float]]:
...
def highT_radius_guard_score_v4(cand, dt_h: float, n_modes: int) -> Tuple[float, Dict[str, float]]:
    """High-temperature single-size radius guard.

    This is deliberately stronger than v4. Ronchi measurements do not provide a
    precise hard target at 2000 K, and Rizk's high-T radius curve is model-derived,
    but in a single-size model an average dislocation-bubble radius of multiple
    micrometres is not acceptable.  The default soft ceiling is therefore
    Rd(2000 K, 1.3% FIMA) <= 800 nm, with an additional penalty on the
    1800->2000 K growth ratio.
    """
    ...
    terms = [
        ...
        max(0.0, r1800 - RD1800_SOFT_MAX_NM) / RD1800_SOFT_MAX_NM,
...
def score_candidate_v4(cand, dt_h: float, n_modes: int, use_full_exp: bool = True):
    if use_full_exp:
    ...
    score_rizk_prior = rizk_prior_score_v4(cand)
    score_partition, score_qgb, part_diag = gas_partition_score_v4(cand, dt_h, n_modes)
    score_radius_guard, radius_diag = highT_radius_guard_score_v4(cand, dt_h, n_modes)
...
def enqueue_known_good_trials_v4(study):
...
def main():
    parser = argparse.ArgumentParser(description="V4 Optuna calibration wrapper with gas partition/q_gb score and stronger high-T radius guard.")
    ...
    parser.add_argument("--radius-guard-weight", type=float, default=0.45)
    ...
    global DV_FIXED, DV_MIN, DV_MAX, W_RIZK_PRIOR, W_PARTITION, W_QGB, W_RADIUS_GUARD, RD2000_MAX_NM, RD_RATIO_MAX, RD1800_SOFT_MAX_NM
    ...
    m.optuna_candidate_from_trial = optuna_candidate_from_trial_v4
    m.score_candidate = score_candidate_v4
    m.enqueue_known_good_trials = enqueue_known_good_trials_v4
    ...
    if args.output_dir:
        ...
        out = os.path.join(
            ...
            args.family,
            f"{dv_tag}_pW{m.W_PRESSURE:g}_pF{m.PRESSURE_FREE_FACTOR:g}_part{W_PARTITION:g}_qgb{W_QGB:g}_prior{W_RIZK_PRIOR:g}_R{RD2000_MAX_NM:g}",
        )

    print("#" * 120)
    print("V4 wrapper settings")
