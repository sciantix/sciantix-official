...
def log_posterior(theta, mean, covariance, params_key, y_real, y_std, points_time, sciantix_folder_path, multiplicative_factor):
    lp = _log_prior(theta, mean, covariance)
    if not np.isfinite(lp):
        return -np.inf
    return lp + _log_likelihood(theta, params_key, y_real, y_std, points_time, sciantix_folder_path, multiplicative_factor)
...
nll = lambda *args: -np.exp(_log_likelihood(*args))
initial_values = np.array([value[0] for value in params_info.values()])
samples = []
...
for i in range(20,len(points_time)):
    ...
    interpolated_data = interpolate(fr_exp_info, points_time[1:i+1])
    fr_real = interpolated_data[:,1]
    fr_std = interpolated_data[:,2]
    soln = minimize(nll, initial_values, args = (keys, fr_real, fr_std,points_time[1:i+1], sciantix_folder_path, False))
    ...
    covariance = np.diag([value[1] ** 2 for value in params_info.values()])
    np.set_printoptions(threshold=np.inf)
    with Pool() as pool:
        sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior, args = (initial_values, covariance, keys,fr_real, fr_std,points_time[1:i+1], sciantix_folder_path, False), backend = backend, pool = pool)
    ...
    brunin = int(2 * np.max(tau))
    thin = int(0.5 * np.min(tau))
    flat_samples = sampler.get_chain(discard = brunin, thin = thin, flat = True)
