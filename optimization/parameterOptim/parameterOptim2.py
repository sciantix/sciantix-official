...
class initialization():
    def __init__(self):
        ...
        print("3: 2nd ramp")
        print("4: key points")
        print("5: feature of 2nd peak in RR")
        print("6: whole range(reduce points)")
        print("7: Both 2 and 3")
        print("8: All(1-6)")
        while True:
            optimStrategy = input("Entering your choice(from 1 to 8):")
            try:
                self.optimInterval = int(optimStrategy)
                if self.optimInterval >= 1 and self.optimInterval <= 8:
                    break
                else:
...
class forInputOutput():
    ...
    def readOutput(self,index_tMaxAtTimeSequence,index_ramp1End, index_ramp2Start, index_ramp2End):
        ...
        self.temperature2_sciantix = self.variable_selected_value[index_ramp2Start:index_ramp2End,1]
    ...
    def getTemperature2(self):
        return self.temperature2_sciantix

def errorFunction(optimInterval,FRexp,FRsciantix,RR1exp,RR1sciantix,RR2exp,RR2sciantix, FR1exp, FR2exp, FR1sciantix, FR2sciantix, temperature2exp, temperature2sciantix):
    ...
    temperature2exp,temperature2sciantix = sizeEqual(temperature2exp,temperature2sciantix)


    if optimInterval == 1:
        ######## two temperature ramps ##################
        errorFR1_twoRamp = abs((np.sum(abs(FR1sciantix)) - np.sum(abs(FR1exp))))/np.sum(abs(FR1exp))
        errorFR2_twoRamp = abs((np.sum(abs(FR2sciantix)) - np.sum(abs(FR2exp))))/np.sum(abs(FR2exp))
        errorRR1_twoRamp = abs((np.sum(abs(RR1sciantix)) - np.sum(abs(RR1exp))))/np.sum(abs(RR1exp))
        errorRR2_twoRamp = abs((np.sum(abs(RR2sciantix)) - np.sum(abs(RR2exp))))/np.sum(abs(RR2exp))
        error_twoRamp = errorFR1_twoRamp + errorFR2_twoRamp +errorRR1_twoRamp + errorRR2_twoRamp
        #####################################
        error = error_twoRamp
    elif optimInterval == 2:
        ######## 1st ramp ##################
        errorFR1_1stRamp = abs((np.sum(abs(FR1sciantix)) - np.sum(abs(FR1exp))))/np.sum(abs(FR1exp))
        errorRR1_1stRamp = abs((np.sum(abs(RR1sciantix)) - np.sum(abs(RR1exp))))/np.sum(abs(RR1exp))
        error_1stRamp = errorFR1_1stRamp + errorRR1_1stRamp
        ####################################
        error = error_1stRamp
    elif optimInterval == 3:
        ######## 2nd ramp #################
        errorFR2_2ndRamp = abs((np.sum(abs(FR2sciantix)) - np.sum(abs(FR2exp))))/np.sum(abs(FR2exp))
        errorRR2_2ndRamp = abs((np.sum(abs(RR2sciantix)) - np.sum(abs(RR2exp))))/np.sum(abs(RR2exp))
        error_2ndRamp = errorFR2_2ndRamp + errorRR2_2ndRamp
        ###################################
        error = error_2ndRamp
    elif optimInterval == 4:
        ##### key points #######
        errorFR1_endPoint = abs((FR1exp[-1]-FR1sciantix[-1])/FR1exp[-1])
        errorFR_endPoint = abs((FRexp[-1] - FRsciantix[-1])/FRexp[-1])
        errorRR1_endPoint = abs((RR1exp[-1]-RR1sciantix[-1])/RR1exp[-1])
        errorRR_max = abs((max(RR2exp)-max(RR2sciantix))/max(RR2exp))
        errorRR2_endPoint = abs((RR2exp[-1]-RR2sciantix[-1])/RR2exp[-1])
        error_keyPoints = errorFR1_endPoint + errorFR_endPoint + errorRR1_endPoint + errorRR_max+ errorRR2_endPoint
        #######################
        error = error_keyPoints
    elif optimInterval == 5:
        ###### Feature of second peak #####
        index_RRPeakExp = np.argmax(RR2exp)
        index_RRPeakSciantix = np.argmax(RR2sciantix)
        index_RRhalfPeakExp = abs(RR2exp[0:index_RRPeakExp] - max(RR2exp)/2).argmin()
        index_RRhalfPeakSciantix = abs(RR2sciantix[0:index_RRPeakSciantix] - max(RR2sciantix)/2).argmin()
        error_FWHM = abs(((temperature2exp[index_RRPeakExp]-temperature2exp[index_RRhalfPeakExp])-(temperature2sciantix[index_RRPeakSciantix]-temperature2sciantix[index_RRhalfPeakSciantix]))/(temperature2exp[index_RRPeakExp]-temperature2exp[index_RRhalfPeakExp]))
        errorRR_max = abs((max(RR2exp)-max(RR2sciantix))/max(RR2exp))
        errorRR2_endPoint = abs((RR2exp[-1]-RR2sciantix[-1])/RR2exp[-1])
        error_2ndPeak = error_FWHM + errorRR2_endPoint + errorRR_max
        ##################################
        error = error_2ndPeak
    else:
        ####### whole range (reduced) ################
        shrinkFactor = 10
        FRexp_reduce = oneDdataSliming(FRexp,shrinkFactor)
        RR1exp_reduce = oneDdataSliming(RR1exp,shrinkFactor)
        RR2exp_reduce = oneDdataSliming(RR2exp,shrinkFactor)
        FRsciantix_reduce = oneDdataSliming(FRsciantix,shrinkFactor)
        RR1sciantix_reduce = oneDdataSliming(RR1sciantix, shrinkFactor)
        RR2sciantix_reduce = oneDdataSliming(RR2sciantix,shrinkFactor)
        errorFR_reduce = abs((np.sum(abs(FRsciantix_reduce))+FR1sciantix[-1]+FRsciantix[-1]) - (np.sum(abs(FRexp_reduce))+FR1exp[-1]+FRexp[-1]))/(np.sum(abs(FRexp_reduce))+FR1exp[-1]+FRexp[-1])
        errorRR1_reduce = abs((np.sum(abs(RR1sciantix_reduce))+RR1sciantix[-1]) - (np.sum(abs(RR1exp_reduce))+RR1exp[-1]))/(np.sum(abs(RR1exp_reduce))+RR1exp[-1])
        errorRR2_reduce = abs((np.sum(abs(RR2sciantix_reduce))+max(RR2sciantix)+RR2sciantix[-1]) - (np.sum(abs(RR2exp_reduce))+max(RR2exp)+RR2exp[-1]))/(np.sum(abs(RR2exp_reduce))+max(RR2exp)+RR2exp[-1])
        error_reduce = errorFR_reduce + errorRR1_reduce + errorRR2_reduce
        #####################################
        error = error_reduce
...
def oneDdataSliming(source_data, shrinkFactor):
    targetData = np.zeros(int(len(source_data)/shrinkFactor)-1)
    for i in range(len(targetData)):
        targetData[i] = source_data[shrinkFactor*i]
    return targetData
...
def afterOptim(optimInterval,results,number_sf_selected,sf_selected,index_ramp1End,index_ramp2Start,index_ramp2End):
    if optimInterval == 1:
        if results.success == True:
            ...
            print(f"two ramps optimization error is:{results.fun}")
        ...
        if results.success == True:
            print("###########################")
            print("Optimization----key points optim finished")
            print("###########################")
            source_input_sf_name = "input_scaling_factors.txt"
            target_input_sf_name = "input_scaling_factors_keyPointsOptim.txt"
            copyToUpperDic_and_rename_file(source_input_sf_name,target_input_sf_name)

            source_output_name = "output.txt"
            target_output_name = "output_keyPointsOptim.txt"
            copyToUpperDic_and_rename_file(source_output_name,target_output_name)
            print("####################################")
            print("#OPTIMIZATION FINISHED, HERE IS YOUR RESULTS:")
            print("####################################")
            for i in range(number_sf_selected):
                print(f"{sf_selected[i]} for key points optim: {results.x[i]}")
            print(f"key Points optim error: {results.fun}")

    elif optimInterval == 5:
        if results.success == True:
            print("###########################")
            print("Optimization----2nd peak feature optim finished")
            print("###########################")
            source_input_sf_name = "input_scaling_factors.txt"
            target_input_sf_name = "input_scaling_factors_2ndPeakFeatureOptim.txt"
            copyToUpperDic_and_rename_file(source_input_sf_name,target_input_sf_name)

            source_output_name = "output.txt"
            target_output_name = "output_2ndPeakFeatureOptim.txt"
            copyToUpperDic_and_rename_file(source_output_name,target_output_name)
            print("####################################")
            print("#OPTIMIZATION FINISHED, HERE IS YOUR RESULTS:")
            print("####################################")
            for i in range(number_sf_selected):
                print(f"{sf_selected[i]} for 2nd peak feature optim: {results.x[i]}")
            print(f"2nd peak feature optim error: {results.fun}")

    else: # optimInterval == 6:
        if results.success == True:
            print("###########################")
            print("Optimization----whole range reduced optim finished")
            print("###########################")
            source_input_sf_name = "input_scaling_factors.txt"
            target_input_sf_name = "input_scaling_factors_wholeRangeReduceOptim.txt"
            copyToUpperDic_and_rename_file(source_input_sf_name,target_input_sf_name)

            source_output_name = "output.txt"
            target_output_name = "output_wholeRangeReduceOptim.txt"
            copyToUpperDic_and_rename_file(source_output_name,target_output_name)
            print("####################################")
            print("#OPTIMIZATION FINISHED, HERE IS YOUR RESULTS:")
            print("####################################")
            for i in range(number_sf_selected):
                print(f"{sf_selected[i]} for whole range reduced optim: {results.x[i]}")
            print(f"whole range reduced optim error: {results.fun}")
...
def optimization(initial,setInputOutput):
    ...
    temperature2exp = initial.getTemperature2exp()
    ...
    def costFunction(sf_selected_value):
        ...
        temperature2sciantix = setInputOutput.getTemperature2()
        ...
        error = errorFunction(optimInterval,FRexp,FRsciantix,RR1exp,RR1sciantix,RR2exp,RR2sciantix, FR1exp, FR2exp, FR1sciantix, FR2sciantix, temperature2exp, temperature2sciantix)
    ...
    elif optimInterval == 7:
        optimInterval_o = optimInterval
        for i in range(4,6): #I want 2 and 3, so 7-4=3, 7-5 =2
    ...
    else: #optimInterval == 8
        optimInterval_o = optimInterval
        for i in range(2,8): #I want 1-6, so 8-2=6, 8-7 = 1
...
def do_plot():
    ...
    ax[0].plot(timeSequence, FRsciantix_optim, 'k',linestyle = '--', label='Optim interval: two ramps')
    ax[0].plot(time1, FR1sciantix, color = 'blue', linestyle ='--', label = 'Optim interval: first ramp')
    ax[0].plot(time2, FR2sciantix, color = '#800080', linestyle ='--', label = 'Optim interval: second ramp')
    ax[0].plot(timeSequence, FRkeyPoint, color = 'gold', linestyle ='--', label = 'Optim interval: key points')
    ax[0].plot(timeSequence, FR2ndPeakFeature, color = 'olive', linestyle ='--', label = 'Optim interval: 2nd peak feature')
    ax[0].plot(timeSequence, FRWholRangeReduce, color = 'deeppink', linestyle ='--', label = 'Optim interval: whole range reduced')
    ...
    ax[1].plot(temperatureSequence, RRsciantix_optim, 'k', linestyle = '--',label='Optim interval: two ramps')
    ax[1].plot(temperature1, RR1sciantix, color = 'blue', linestyle ='--', label = 'Optim interval: first ramp')
    ax[1].plot(temperature2, RR2sciantix, color = '#800080', linestyle ='--', label = 'Optim interval: second ramp')
    ax[1].plot(temperatureSequence, RRkeyPoint, color = 'gold', linestyle ='--', label = 'Optim interval: key points')
    ax[1].plot(temperatureSequence, RR2ndPeakFeature, color = 'olive', linestyle ='--', label = 'Optim interval: 2nd peak feature')
    ax[1].plot(temperatureSequence, RRWholRangeReduce, color = 'deeppink', linestyle ='--', label = 'Optim interval: whole range reduced')
