...
Purpose of v6:
  ...
  - keep v5 scoring and add v6 physics-split parameterization so R_d(T) cannot keep accelerating after ~1800 K.

Recommended first use:
  python UN_M7_optuna_calibration_v6.py --family capture_only --dv-min 0.2 --dv-max 2.0 \
...
def optuna_candidate_from_trial_v5(trial, family: str):
    vals = {
        ...
        "b_scale": 1.0,
        "gd_scale": 1.0,
        "D2_xe_scale": trial.suggest_float("D2_xe_scale", 0.01, 50.0, log=True),
...
def main():
    parser = argparse.ArgumentParser(description="V6 Optuna calibration wrapper with gas partition/q_gb score and explicit high-T radius saturation guard.")
