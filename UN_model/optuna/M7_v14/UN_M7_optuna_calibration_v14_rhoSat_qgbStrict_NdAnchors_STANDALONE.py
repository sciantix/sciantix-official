...
Truly standalone V14 calibration script. It embeds the M7/v8 physics core,
the v8 guardrail scoring helpers reused by v14, and the v14 rhoSat/qgb/Nd
anchor objective in a single file.
"""
...
# ======================================================================================
# Embedded v8 core: M7 physics model, data, scoring utilities, Optuna runner, and I/O.
# ======================================================================================
# UN M7 v6 CORE - capture_only physics-split model / standalone script
#
# Purpose:
...
class Candidate:
    ...
    # Backward-compatible aggregate scales.
    # In v6 Optuna runs these are normally kept at 1.0, while the split scales below are sampled.
    Dv_scale: float = 1.0
...
def _row_float(row: Dict, key: str, default: float) -> float:
    try:
...
def candidate_from_score_row(row: Dict, label_prefix: str = "best_M7") -> Candidate:
    # Backward compatible: old v5 rows without split v6 columns are mapped so that
    # the old aggregate factors are reproduced by the new split fields.
    old_Dg = _row_float(row, "Dg_scale", 1.0)
    old_Dv = _row_float(row, "Dv_scale", 1.0)
    old_b = _row_float(row, "b_scale", 1.0)
    old_gd = _row_float(row, "gd_scale", 1.0)
    return Candidate(
        ...
        Dv_scale=_row_float(row, "Dv_scale", 1.0),
        Dg_scale=_row_float(row, "Dg_scale", 1.0),
        b_scale=_row_float(row, "b_scale", 1.0),
        gb_scale=_row_float(row, "gb_scale", 1.0),
        gd_scale=_row_float(row, "gd_scale", 1.0),
        coalescence_d_scale=float(row["coalescence_d_scale"]),
        capture_scale=float(row["capture_scale"]),
        D2_xe_scale=_row_float(row, "D2_xe_scale", 1.0),
        Dg_D1_scale=_row_float(row, "Dg_D1_scale", 1.0),
        Dg_D3_scale=_row_float(row, "Dg_D3_scale", 1.0),
        Dv_D1_scale=_row_float(row, "Dv_D1_scale", 1.0),
        Dv_D2_scale=_row_float(row, "Dv_D2_scale", 1.0),
        b_bulk_scale=_row_float(row, "b_bulk_scale", 1.0),
        b_dislocation_scale=_row_float(row, "b_dislocation_scale", 1.0),
        gd_bubble_scale=_row_float(row, "gd_bubble_scale", 1.0),
        gd_line_scale=_row_float(row, "gd_line_scale", 1.0),
        gd_line_alpha=_row_float(row, "gd_line_alpha", 1.0),
    )
...
def optuna_candidate_from_trial(trial, family: str):
    vals = {
        ...
        # Aggregate scales kept at 1.0 in v6 optimization.
        "Dv_scale": 1.0,
        "Dg_scale": 1.0,
        "b_scale": 1.0,
        "gd_scale": 1.0,
    ...
    return Candidate(label=f"{family}_v6_trial_{trial.number:05d}", **vals)
...
# ======================================================================================
# Embedded v8 guardrail helpers reused by v14.
# ======================================================================================
DV_FIXED = None
DV_MIN = 0.20
DV_MAX = 2.00
...
# v7 additions: explicitly score remaining weak points.
W_ND_DROP_TARGET = 0.55
W_BULK_PLATEAU = 0.55
W_HIGHT_PRESSURE = 0.45
W_RADIUS_BAND = 0.75
...
def optuna_candidate_from_trial_v5(trial, family: str):
    vals = {
        ...
        # v6: keep old aggregate factors at 1.0 and sample split physical terms.
        "Dv_scale": 1.0,
        "Dg_scale": 1.0,
        "b_scale": 1.0,
        "gd_scale": 1.0,
...
def rizk_prior_score_v5(cand) -> float:
    # v6 prior: old aggregate scales are not sampled; split scales are penalized.
    # f_n and coalescence/capture remain weak closure priors.
    items = [
        (cand.Dg_D1_scale, 1.0, 0.45),
        (cand.Dg_D3_scale, 1.0, 0.45),
        (cand.Dv_D1_scale, 1.0, 0.25),
        (cand.Dv_D2_scale, 1.0, 0.25),
        (cand.b_bulk_scale, 1.0, 0.45),
        (cand.b_dislocation_scale, 1.0, 0.55),
        (cand.gb_scale, 1.0, 0.35),
        (cand.gd_bubble_scale, 1.0, 0.30),
        (cand.gd_line_scale, 1.0, 0.30),
        # gd_line_alpha is a model-form interpolation, not a log scale.
        (max(0.05, min(1.0, cand.gd_line_alpha if cand.gd_line_alpha > 0 else 0.05)), 1.0, 0.12),
        (cand.K_d, K_D_NOMINAL, 0.35),
        (cand.rho_d, RHO_D_NOMINAL, 0.35),
        (cand.fission_rate, FISSION_RATE_NOMINAL, 0.25),
        (cand.f_n, F_N_NOMINAL, 0.08),
...
def gas_partition_score_v5(cand, dt_h: float, n_modes: int) -> Tuple[float, float, Dict[str, float]]:
    ...
    for bu, T, min_bulk, min_disl, max_bulk, max_qgb, tag in anchors:
        out = run_model_point(T, bu, cand, dt_h, n_modes, keep_history=False)
    ...
    for bu in (1.1, 3.2):
        out1500 = run_model_point(1500.0, bu, cand, dt_h, n_modes, keep_history=False)
        out2000 = run_model_point(2000.0, bu, cand, dt_h, n_modes, keep_history=False)
...
def highT_radius_guard_score_v5(cand, dt_h: float, n_modes: int) -> Tuple[float, float, Dict[str, float]]:
    ...
    """
    out1600 = run_model_point(1600.0, 1.3, cand, dt_h, n_modes, keep_history=False)
    out1700 = run_model_point(1700.0, 1.3, cand, dt_h, n_modes, keep_history=False)
    out1800 = run_model_point(1800.0, 1.3, cand, dt_h, n_modes, keep_history=False)
    out1900 = run_model_point(1900.0, 1.3, cand, dt_h, n_modes, keep_history=False)
    out2000 = run_model_point(2000.0, 1.3, cand, dt_h, n_modes, keep_history=False)
...
def nd_coalescence_shape_score_v7(cand, dt_h: float, n_modes: int):
    """Encourage high-T N_d decrease without directly forcing a huge radius."""
    outs = {}
    for T in (1400.0, 1550.0, 1600.0, 1650.0, 1725.0, 1800.0):
        outs[T] = run_model_point(T, 1.3, cand, dt_h, n_modes, keep_history=False)

    def lr(Ta, Tb):
        return safe_log10_ratio(outs[Ta]["Nd"], outs[Tb]["Nd"])

    d1600_1400 = lr(1600.0, 1400.0)
    d1650_1400 = lr(1650.0, 1400.0)
    d1725_1400 = lr(1725.0, 1400.0)
    d1800_1400 = lr(1800.0, 1400.0)
    d1725_1550 = lr(1725.0, 1550.0)

    terms = [
        max(0.0, d1600_1400 - (-0.15)),
        max(0.0, d1650_1400 - (-0.30)),
        max(0.0, d1725_1400 - (-0.60)),
        0.7 * max(0.0, d1800_1400 - (-0.80)),
        max(0.0, d1725_1550 - (-0.35)),
        0.5 * max(0.0, (-1.20) - d1600_1400),
    ]

    score = _rmse(terms)
    return score, {
        "score_Nd_coalescence_shape_terms_rmse": score,
        "Nd_drop_1600_over_1400_log10_v7": d1600_1400,
        "Nd_drop_1650_over_1400_log10_v7": d1650_1400,
        "Nd_drop_1725_over_1400_log10_v7": d1725_1400,
        "Nd_drop_1800_over_1400_log10_v7": d1800_1400,
        "Nd_drop_1725_over_1550_log10_v7": d1725_1550,
        "Nd_1p3_1550K_v7": outs[1550.0]["Nd"],
        "Nd_1p3_1650K_v7": outs[1650.0]["Nd"],
        "Nd_1p3_1800K_v7": outs[1800.0]["Nd"],
    }


def bulk_plateau_score_v7(cand, dt_h: float, n_modes: int):
    """Keep a Rizk-like bulk-bubble gas plateau before the high-T transition."""
    anchors = [
        (1.1, 1200.0, 72.0, 8.0, "1p1_1200"),
        (1.1, 1400.0, 72.0, 8.0, "1p1_1400"),
        (1.1, 1500.0, 70.0, 9.0, "1p1_1500"),
        (1.1, 1600.0, 66.0, 10.0, "1p1_1600"),
        (3.2, 1400.0, 82.0, 8.0, "3p2_1400"),
        (3.2, 1500.0, 80.0, 9.0, "3p2_1500"),
        (3.2, 1600.0, 74.0, 11.0, "3p2_1600"),
    ]
    terms = []
    diag = {}
    for bu, T, min_bulk, max_qgb, tag in anchors:
        out = run_model_point(T, bu, cand, dt_h, n_modes, keep_history=False)
        bulk = out["bulk_gas_percent"]
        disl = out["dislocation_gas_percent"]
        qgb = out["qgb_gas_percent"]
        diag[f"bulk_plateau_bulk_{tag}"] = bulk
        diag[f"bulk_plateau_disl_{tag}"] = disl
        diag[f"bulk_plateau_qgb_{tag}"] = qgb
        terms.append(max(0.0, min_bulk - bulk) / 65.0)
        terms.append(0.6 * max(0.0, qgb - max_qgb) / 40.0)

    out2000 = run_model_point(2000.0, 1.1, cand, dt_h, n_modes, keep_history=False)
    terms.append(max(0.0, 88.0 - out2000["dislocation_gas_percent"]) / 70.0)
    diag["bulk_plateau_disl_1p1_2000"] = out2000["dislocation_gas_percent"]
    diag["bulk_plateau_bulk_1p1_2000"] = out2000["bulk_gas_percent"]
    diag["bulk_plateau_qgb_1p1_2000"] = out2000["qgb_gas_percent"]

    return _rmse(terms), diag


def highT_pressure_score_v7(cand, dt_h: float, n_modes: int):
    """Add pressure checks where v6 allowed p_d/p_eq to rise at high T."""
    anchors = [
        (3.2, 1700.0, 2.5, "3p2_1700"),
        (3.2, 1800.0, 2.7, "3p2_1800"),
        (3.2, 1900.0, 3.0, "3p2_1900"),
        (3.2, 2000.0, 3.0, "3p2_2000"),
        (1.1, 1900.0, 2.7, "1p1_1900"),
        (1.1, 2000.0, 3.0, "1p1_2000"),
    ]
    terms = []
    diag = {}
    for bu, T, max_ratio, tag in anchors:
        out = run_model_point(T, bu, cand, dt_h, n_modes, keep_history=False)
        ratio = out["p_d_over_eq"]
        diag[f"pd_over_eq_highT_{tag}"] = ratio
        diag[f"pb_over_eq_highT_{tag}"] = out["p_b_over_eq"]
        if math.isfinite(ratio):
            terms.append(max(0.0, ratio - max_ratio) / max_ratio)
        else:
            terms.append(10.0)
...
def radius_band_score_v8(cand, dt_h: float, n_modes: int):
    """Softly prefer a high-T radius band instead of only imposing upper guards.

    Motivation:
    v7 often selected R_d(2000 K) ≈ 210-280 nm.  That is safe but probably too
    low if we want enough coalescence/N_d drop and a Rizk-like high-T asymptote.
    This term asks whether a compromise with R_d(2000 K) roughly 350-650 nm
    exists, while still allowing the experimental-range fit to dominate.
    """
    outs = {}
    for T in (1700.0, 1800.0, 1900.0, 2000.0):
        outs[T] = run_model_point(T, 1.3, cand, dt_h, n_modes, keep_history=False)

    r1700 = outs[1700.0]["Rd_nm"]
    r1800 = outs[1800.0]["Rd_nm"]
    r1900 = outs[1900.0]["Rd_nm"]
    r2000 = outs[2000.0]["Rd_nm"]

    def below(x, lo, scale):
        return max(0.0, lo - x) / scale

    def above(x, hi, scale):
        return max(0.0, x - hi) / scale

    # Soft band, not a hard target.  We mainly penalize too-low high-T radii,
    # because the existing radius guard already penalizes too-high radii.
    terms = [
        below(r1800, 170.0, 350.0),
        below(r1900, 250.0, 350.0),
        below(r2000, 350.0, 350.0),
        0.35 * above(r1900, 650.0, 650.0),
        0.35 * above(r2000, 750.0, 750.0),
    ]

    # Prefer continued growth between 1800 and 2000 if the curve is still too low,
    # but do not reward runaway acceleration.
    inc_18_20 = max(0.0, r2000 - r1800)
    terms.append(0.5 * below(inc_18_20, 120.0, 350.0))

    score = _rmse(terms)
    return score, {
        "score_radius_band_terms_rmse": score,
        "Rd_band_1700_nm_v8": r1700,
        "Rd_band_1800_nm_v8": r1800,
        "Rd_band_1900_nm_v8": r1900,
        "Rd_band_2000_nm_v8": r2000,
        "Rd_band_inc_1800_2000_nm_v8": inc_18_20,
    }
...
def score_candidate_v5(cand, dt_h: float, n_modes: int, use_full_exp: bool = True):
    ...
    score_Nd_coalescence_shape, nd_shape_diag = nd_coalescence_shape_score_v7(cand, dt_h, n_modes)
    score_bulk_plateau, bulk_plateau_diag = bulk_plateau_score_v7(cand, dt_h, n_modes)
    score_highT_pressure, highT_pressure_diag = highT_pressure_score_v7(cand, dt_h, n_modes)
    score_radius_band, radius_band_diag = radius_band_score_v8(cand, dt_h, n_modes)

    # Keep the base high-T guard weak as well; it catches extreme divergence.
    score_base_highT_guard = highT_guard_score(cand, dt_h, n_modes)
    ...
    total = (
        ...
        + W_ND_DROP_TARGET * score_Nd_coalescence_shape
        + W_BULK_PLATEAU * score_bulk_plateau
        + W_HIGHT_PRESSURE * score_highT_pressure
        + W_RADIUS_BAND * score_radius_band
    ...
    result = {
        **candidate_to_dict(cand),
        ...
        "score_bulk_plateau": score_bulk_plateau,
        "score_highT_pressure": score_highT_pressure,
        "score_radius_band": score_radius_band,
...
ORIGINAL_ENQUEUE_KNOWN_GOOD_TRIALS = enqueue_known_good_trials
...
# ======================================================================================
# V14 objective and CLI.
# ======================================================================================
_ORIGINAL_RUN_MODEL_POINT = run_model_point
_ORIGINAL_CANDIDATE_FROM_SCORE_ROW = candidate_from_score_row
...
def optuna_candidate_from_trial_v10c(trial, family: str):
    ...
    cand = Candidate(label=f"{family}_v13_trial_{trial.number:05d}", **vals)
...
def candidate_from_score_row_v10c(row: Dict, label_prefix: str = "best_M7"):
    ...
    rho_scale = safe_float(row.get("rho_scale", row.get("rho_global_scale", 1.0)), 1.0)
    RHO_SCALE_BY_LABEL[cand.label] = rho_scale
    return cand
...
def enqueue_known_good_trials_v10c(study):
    ...
    try:
        enqueue_known_good_trials_v5(study)
    except Exception:
        pass
    for k in [
...
def rizk_fig6_dislocation_burnup_score(cand, dt_h: float, n_modes: int) -> Tuple[float, Dict[str, float]]:
    ...
    for bu, target in RIZK_FIG6_DISL_ANCHORS:
        out = run_model_point(1600.0, bu, cand, dt_h, n_modes, keep_history=False)
...
def score_candidate_v10c(cand, dt_h: float, n_modes: int, use_full_exp: bool = True):
    row = score_candidate_v5(cand, dt_h, n_modes, use_full_exp=use_full_exp)
...
This standalone file embeds the M7/v8 core and the v8 scoring helpers it uses.
"""
...
def rizk_fig8_radius_score(cand, dt_h: float, n_modes: int):
    ...
    for T, target in RIZK_FIG8_DISLOCATION_RADIUS_SCORE_POINTS:
        out = run_model_point(T, 1.3, cand, dt_h, n_modes, keep_history=False)
        pred = out["Rd_nm"]
...
def gas_partition_score_v13(cand, dt_h: float, n_modes: int):
    ...
    def get(bu: float, T: float):
        ...
        if key not in cache:
            cache[key] = run_model_point(T, bu, cand, dt_h, n_modes, keep_history=False)
        return cache[key]
...
def nd_highT_low_target_score_v13(cand, dt_h: float, n_modes: int):
    ...
    """
    diag: Dict[str, float] = {}
    out1400 = run_model_point(1400.0, 1.3, cand, dt_h, n_modes, keep_history=False)
    out1900 = run_model_point(1900.0, 1.3, cand, dt_h, n_modes, keep_history=False)
    out2000 = run_model_point(2000.0, 1.3, cand, dt_h, n_modes, keep_history=False)
...
def score_candidate_v13(cand, dt_h: float, n_modes: int, use_full_exp: bool = True):
    # Direct experimental data from core/
    if use_full_exp:
        sw_points = [p for p in EXP_SWELLING_T if p["T"] <= 1700.0]
        nd_points = EXP_ND_T_13
        rd_points = EXP_RD_T_13
    ...
    for exp in rd_points:
        out = run_model_point(exp["T"], 1.3, cand, dt_h, n_modes, keep_history=False)
        r_pairs.append((out["Rd_nm"], exp["R_nm"]))
    score_Rd_exp = log10_rmse(r_pairs)
    ...
    score_radius_guard, score_radius_saturation, radius_guard_diag = highT_radius_guard_score_v5(cand, dt_h, n_modes)
    score_Nd_coalescence_shape, nd_shape_diag = nd_coalescence_shape_score_v7(cand, dt_h, n_modes)
...
def main():
    ...
    # Direct experimental weights.
    W_SWELLING = args.exp_swelling_weight
    W_RD = args.exp_rd_weight
    W_ND_LEVEL = args.exp_nd_level_weight
    W_ND_DROP = args.base_nd_drop_weight
    W_PRESSURE = args.pressure_weight
    PRESSURE_FREE_FACTOR = args.pressure_free_factor

    # v8 guard/prior globals reused by v13.
    W_RIZK_PRIOR = args.rizk_prior_weight
    W_RADIUS_GUARD = args.radius_guard_weight
    W_ND_DROP_TARGET = args.nd_drop_target_weight
    W_HIGHT_PRESSURE = args.highT_pressure_weight
    ...
    run_model_point = v10c.run_model_point_rhoFT2
    optuna_candidate_from_trial = v10c.optuna_candidate_from_trial_v10c
    candidate_from_score_row = v10c.candidate_from_score_row_v10c
    score_candidate = score_candidate_v13
    enqueue_known_good_trials = v10c.enqueue_known_good_trials_v10c
    ...
    print(f"direct exp weights              = sw {W_SWELLING}, Rd {W_RD}, Nd {W_ND_LEVEL}, Nd-drop {W_ND_DROP}")
