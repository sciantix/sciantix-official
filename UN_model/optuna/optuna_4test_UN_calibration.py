#!/usr/bin/env python3
"""
Optuna calibration runner for 4test_UN_rhoRizk2023.ipynb.
...
Free parameters in this first calibration:
  - rho_mode: constant / blank_sat / blank_sat_deformed
...
Fixed parameters:
  - gb_scale = gd_scale = b_scale = capture_scale = coalescence_d_scale = 1
  - Dg/Dv bulk scales = 1
...
def load_notebook_model(notebook_path: Path, output_dir: Path, dt_h: float, n_modes: int) -> Dict[str, Any]:
    ...
    # Keep current intended physics switches from the notebook unless user changes notebook itself.
    # The Optuna script only changes params listed in suggest_trial().
    return ns
...
def make_effective_rho_function(ns: Dict[str, Any], rho_cfg: Dict[str, Any], rho_fab_base: float, rho_max: float):
    ...
    def effective_rho_d(T: float, burnup: float, cand) -> float:
        ...
        return min(max(rho, 1.0e10), float(rho_max))
...
def suggest_trial(trial, ns: Dict[str, Any], args) -> Tuple[Any, Dict[str, Any]]:
    rho_mode = trial.suggest_categorical("rho_mode", ["constant", "blank_sat", "blank_sat_deformed"])
    ...
    # Fixed physical/scaling choices for this first clean Optuna.
...
def score_swelling_T(ns: Dict[str, Any], runner: TrialRunner) -> float:
    ...
    for p in ns["EXP_SWELLING_T"]:
        ...
        vals.append(logerr(r["swelling_d_percent"], target))
...
def score_radius_T(ns: Dict[str, Any], runner: TrialRunner) -> float:
    ...
    for p in ns["EXP_RD_T_13"]:
        ...
        vals.append(logerr(r["Rd_nm"], target))
    return float(np.mean(vals)) if vals else 0.0
...
def write_trial_outputs(study, ns: Dict[str, Any], args):
    ...
    lines.append("- `capture_scale = 1.0`\n")
    ...
    if best.params["rho_mode"] == "constant":
        rho_d = float(ns["RHO_D_NOMINAL"]) * float(best.params["rho_const_scale"])
        lines.append(f'    "rho_d": {rho_d:.12e},\n')
    else:
        lines.append(f'    "rho_d": {float(ns["RHO_D_NOMINAL"]):.12e},  # not used directly for {best.params["rho_mode"]}\n')
...
def parse_args(argv=None):
    ...
    parser.add_argument("--notebook", type=Path, default=Path("4test_UN_rhoRizk2023.ipynb"))
    parser.add_argument("--output-dir", type=Path, default=Path("optuna_4test_UN_results"))
    parser.add_argument("--study-name", type=str, default="optuna_4test_UN_clean_rho")
...
def main(argv=None):
    ...
    print(f"Starting Optuna: n_trials={args.n_trials}, dt_h={args.dt_h}, n_modes={args.n_modes}")
