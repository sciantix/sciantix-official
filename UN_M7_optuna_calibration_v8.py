...
Purpose of v8:
  ...
  - keep v6 physics split, v7 Nd/bulk/pressure scoring, and add high-T radius band scoring so R_d(T) cannot keep accelerating after ~1800 K.

Recommended first use:
  python UN_M7_optuna_calibration_v8.py --family capture_only --dv-min 0.2 --dv-max 2.0 \
...
def optuna_candidate_from_trial_v5(trial, family: str):
    ...
    return m.Candidate(label=f"{family}_v6_trial_{trial.number:05d}", **vals)
...
def nd_coalescence_shape_score_v7(cand, dt_h: float, n_modes: int):
    ...
    for T in (1400.0, 1550.0, 1600.0, 1650.0, 1725.0, 1800.0):
        outs[T] = m.run_model_point(T, 1.3, cand, dt_h, n_modes, keep_history=False)

    def lr(Ta, Tb):
        return m.safe_log10_ratio(outs[Ta]["Nd"], outs[Tb]["Nd"])
...
def bulk_plateau_score_v7(cand, dt_h: float, n_modes: int):
    ...
    out2000 = m.run_model_point(2000.0, 1.1, cand, dt_h, n_modes, keep_history=False)
...
def main():
    ...
    global W_ND_DROP_TARGET, W_BULK_PLATEAU, W_HIGHT_PRESSURE, W_RADIUS_BAND
    ...
    W_BULK_PLATEAU = args.bulk_plateau_weight
    W_HIGHT_PRESSURE = args.highT_pressure_weight
    W_RADIUS_BAND = args.radius_band_weight
    ...
    print(f"Nd drop target weight    = {W_ND_DROP_TARGET}")
    print(f"bulk plateau weight      = {W_BULK_PLATEAU}")
    print(f"highT pressure weight    = {W_HIGHT_PRESSURE}")
    print(f"radius band weight       = {W_RADIUS_BAND}")
    ...
    print("capture law             = v6 Barani dR_d geometry; active for capture_only and M7_no_phi")
