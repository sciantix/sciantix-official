...
| Percolation + mechanical saturation factor | `F_sat = (1−ξ_old/ξ_sat)²`, **`ξ_sat = 0.18`** | `HighBurnupStructurePorosity.C` case 2 line 98 (and case 3 line 430) | Stauffer-Aharony `t=2` applied to `D_gb^v` (grain-boundary percolation) **and** post-hoc to the total pore-volume increment ΔV (mechanical stress on residual solid cross-section). **Not** applied to `D_gb^SA`/β_n. Coalescence receives the capped ΔV. Lowered from 0.22 → 0.20 → 0.18 in §15.15 to keep the asymptote inside the Spino-Cappia envelope after the σ_h rampup change. |
...
1. **Grain-boundary percolation (Stauffer-Aharony `t=2`)**: `D_gb_v_eff = D_gb^v · tilt_factor · F_sat`, used in `volume_flow_rate = 2π ρ_P D_gb_v_eff / ζ(ψ)`. Schottky-vacancy transport along grain boundaries propagates through the connected solid GB network (the percolation backbone in the Stauffer-Aharony sense, i.e. the connected subnet of the solid phase); as the network fragments near `ξ_sat`, vacancy flow shuts down.
...
1. **Grain-boundary percolation.** Schottky-vacancy transport
   along grain boundaries propagates through the connected solid GB
   network (the percolation backbone in the Stauffer-Aharony sense — the
   connected subnet of the solid phase, NOT a "vacancy backbone").
   As ξ → ξ_sat, the network fragments and vacancy flow shuts down.
...
| §3.6 text | Two-channel saturation: (i) grain-boundary percolation on `D_gb^v`, (ii) mechanical cap on ΔV. EoS identity `V_p = n_Xe Ω_Xe + n_vp Ω_vac` **is broken under saturation by design** (over-pressurisation). | Replaces the "applied directly to both grain-boundary diffusivities" paragraph. |
...
19. **`backbone` and `hop` are technical terms — keep them, but never use the compound `vacancy-backbone percolation`.** `backbone` (percolation theory: the connected subnet of the **solid phase** that carries flow, after Stauffer-Aharony) and `hop` (solid-state diffusion: elementary atomic jump between adjacent sites) are specialist vocabulary anchored in the cited literature. Standalone uses such as "the connected solid backbone" / "bulk backbone connectivity" are correct and kept. **However**, the compound `vacancy-backbone percolation` is wrong: it suggests the backbone is made of vacancies, when in S&A the backbone is the connected solid phase along which the vacancies hop. Use **`grain-boundary percolation`** instead. The two-channel saturation mechanism is therefore: (i) grain-boundary percolation on `D_gb^v`, (ii) mechanical cap on ΔV. See §17.1 (originally introduced in §15.9, amended in §17).
...
16. **Irradiation duration extended 74 000 h → 84 000 h** in both `test_UO2HBS/input_history.txt` and `test_UO2HBS_0/input_history.txt`. EoL burnup goes from ~201 → ~229 MWd/kgHM. σ_h ramp endpoint unchanged at −70 MPa, so the ramp is slightly slower (more time spent at low compressive stress per unit burnup increment). Curve shape preserved: same low-burnup behaviour, N_p peak shifts marginally to higher burnup (~125 vs ~100-125 MWd/kgHM previously, value still ~5.1×10¹⁷), porosity asymptote essentially unchanged at ξ ≈ 0.16-0.17. §6 expected-baseline table updated to include the bu = 225 MWd/kgHM row. Test `test_UO2HBS` gold file in sync; `test_UO2HBS_0` gold also in sync (refreshed in a later session — verified 2026-04-29). Test `test_UO2HBS_dislocation` (out-of-scope per §13.10) was not modified — still 74 000 h, σ_h = −20 MPa constant.
17. **CV-plot saw-tooth oscillation acknowledged via figure-caption footnote** ([main.tex Fig. fig:CVResults caption]). Oscillations visible in the `plot_CV.png` over bu ≈ 22-30 MWd/kgHM, alternating step-to-step between CV ≈ 0.5-0.7 and 0.9-1.0 with amplitude ~0.2-0.3. Diagnosis: kinematic-broadening source `(n̄ − 2)² · ν_P · dt` on `dB/dt` is sensitive to `n̄ = A/N_p` at pore-nucleation onset where N_p is small (~10¹¹-10¹² m⁻³); implicit-Euler coupling between A and N_p produces alternating-sign perturbations in M₂ when dt is too large.

    **Attempted fix tried and reverted**: raised `Number_of_time_steps_per_interval` 7 500 → 30 000 in `src/MainVariables.C:28` (dt 11.2 h → 2.8 h), rebuilt, reran both tests and refreshed golds. Result was insufficient: residual saw-tooth still visible in the plot. Reverted to 7 500 (original baseline restored, golds refreshed again). A more aggressive fix (60 000 steps, or moment-equation substepping at low N_p) is left as a future task.

    **Submitted-paper choice**: figure caption of Fig. `fig:CVResults` carries an explicit footnote acknowledging the artefact: *"Step-to-step oscillations visible in the bu ≈ 22-30 MWd/kgHM range are a known numerical artefact of the implicit-Euler moment solver near nucleation onset, where the population is small and the kinematic-broadening source on dB/dt dominates; they self-damp once N_p grows above ~10¹³ m⁻³ and do not affect the integrated quantities (ξ, R_p, ⟨N_p⟩)."* Honest disclosure rather than silent smoothing. Both gold files re-refreshed at the 7 500-step baseline; §6 expected-baseline table values unchanged.
18. **CV notation unified to `√(B/N_p) / n̄` everywhere** ([regression_hbs.py:580, 584], [main.tex:499]). Previously the CV plot used three different equivalent forms: y-axis `σ_n / n̄`, legend `√M_2 / n̄`, paper §3.5 prose and Fig 4 caption `√(B/N_p) / n̄`. All three are mathematically identical (`σ_n = √M_2 = √(B/N_p)`) but reading three notations in one section is confusing. Canonical form chosen: `√(B/N_p) / n̄` — already in the paper text, ties directly to moment B from Eq. 17. Plot regenerated. Single-curve legend dropped (redundant with the y-axis label after unification). Style rule §13.20 entry implicit: keep one symbol per concept across plots, captions, and prose.
...
## 17. Recent-session change log (2026-04-30)

1. **`vacancy-backbone percolation` → `grain-boundary percolation`** in paper, code comments, and CONTEXT.md. The compound term was incorrect: in Stauffer-Aharony, the **backbone** is the connected subnet of the **solid phase** along which flow propagates, not the vacancy population. Saying "vacancy-backbone" implied the backbone was made of vacancies. Replaced in:
   - `main.tex:43` (abstract), `main.tex:494` (Discussion §6.1), `main.tex:551` (Conclusions).
   - `HighBurnupStructurePorosity.C:87-89` (header comment for the saturation block); also reworded "Schottky-vacancy transport along the backbone shuts down" → "...along the GB network shuts down" inside the same comment for consistency.
   - CONTEXT.md §2 line 42 (parameter table), §3 line 85 (saturation channel description), §4 line 224 (physics rationale, lines 224-227 reworded), §9 line 382 (paper-equation alignment table).
   Standalone uses of "backbone" in the correct S&A sense (e.g. `main.tex:334` "the connected solid backbone", `main.tex:344` "bulk connectivity of the solid backbone", `main.tex:406` "percolation of the solid backbone", `HighBurnupStructurePorosity.C:94` "bulk backbone connectivity") are kept, since they correctly describe the connected solid phase as the percolation backbone. Style rule §13.19 amended accordingly: keep `backbone` standalone, but never write `vacancy-backbone percolation`. The §15.9 decision to retain "backbone" as jargon is preserved; only the compound name of the channel is changed.

6. **Pre-submission audit pass** (cover letter + code + paper + submission package). Build clean (zero warnings); both regression golds (`test_UO2HBS` and `test_UO2HBS_0`) match current output. Four undefined bib citations — `olanderLightWaterReactor2017`, `olanderLightWaterReactor2021`, `cappellariPhysicsbasedModelGrainboundary2025`, `baraniAnalysisTransientFission2017` — were missing from `HBS.bib` and added by the user during the session. Highlights H3 and H5 trimmed below the NED 85-char hard limit (final lengths 78/47/66/67/71). Code-side residue cleaned: (a) stale comment block at `HighBurnupStructurePorosity.C:85-96` rewritten — it described the abandoned symmetric two-pathway formulation as if current; replaced with the production formulation (F_sat on D_gb^v + post-hoc dV cap, NOT on β_n); (b) misleading "TEST VARIANT (Frattini-style)" header at `HighBurnupStructurePorosity.C:175-179` re-labelled neutrally; (c) reference string at `HighBurnupStructureFormation.C:66` changed `calibration Zullo (2026)` → `parameter selection Zullo (2026)` and `MWd/kgU` → `MWd/kgHM` per Style rules §13.11 / §12.4 (string is reader-facing in `output.txt`); (d) three exploratory commented-out D_gb^SA expressions (Liu 2023 tilt S5, random GB, twist S5) removed from `Matrix.C` case 1; the documented Olander-Van Uffelen "low D" catch-22 block is kept in compressed form. Paper-side: ν_P prefactor wording softened `recalibrated` → `rescaled` at `main.tex:248,393` for tonal consistency with the bu_inc framing rule (§13.11). Highlights .docx synced. Final build status: 41 pages, 0 undefined references, 6 cosmetic overfulls (all <18 pt).